...
import { getGameAssetUrl, getImageUrl } from './gameAssets';
...
export function getOrCreateWeekFeedState(gameState: GameState, langCode: string): UnifiedFeedState {
  ...
  const sameRegionTeams = gameState.teams.filter(t => t.region === playerRegion && t.id !== playerTeam?.id);
  const rivalTeam = sameRegionTeams.sort((a, b) => (b.popularity || 0) - (a.popularity || 0))[0] || { name: 'paiN Gaming', acronym: 'PNG' };
  const rivalName = rivalTeam.name;
  const rivalAcronym = rivalTeam.acronym;
    ...
    pt: [
      {
        ...
        content: `O clássico @${teamAcronym} contra a ${rivalAcronym} nessa semana vai parar o cenário competitivo de eSports! Disputa acirradíssima pelo topo da liga. Quem perder vai aguentar sarro da torcida rival. Qual seu palpite? 🔥`,
      ...
      },
      {
        ...
        content: `@${teamAcronym} contra ${rivalAcronym} nos palcos... se vacilarem na fase de rotas ou errarem o pick de escala do patch, já era. Esse duelo de liderança com ${midPlayer} vai ser decidido nos detalhes técnicos! 👊`,
    ...
    en: [
      {
        ...
        verified: true,
        avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=100',
        content: `The classic match of @${teamAcronym} vs ${rivalAcronym} this week will stop the competitive eSports scene! Extremely fierce battle for the top. What is your guess? 🔥`,
        likes: 8500 + weekNum * 123,
        retweets: 3200 + weekNum * 47,
        timeAgo: '23 min ago',
        sentiment: 'neutral' as const
      },
      {
        ...
        content: `@${teamAcronym} vs ${rivalAcronym} on stage... if they slip up in laning phase or select the wrong draft champ, it's over. This duel with midlaner ${midPlayer} is decided on details! 👊`,
        likes: 9340 + weekNum * 111,
        retweets: 4100 + weekNum * 39,
        timeAgo: '45 min ago',
        sentiment: 'positive' as const
    ...
    es: [
      {
        ...
        content: `¡El partido clásico de @${teamAcronym} contra ${rivalAcronym} questa semana parará la escena competitiva! Una batalla feroz por la cima de la tabla. ¿Cuál es tu pronóstico? 🔥`,
        likes: 8500 + weekNum * 123,
        retweets: 3200 + weekNum * 47,
        timeAgo: 'hace 23 min',
        sentiment: 'neutral' as const
      },
      {
        ...
        verified: true,
        avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=80&q=80',
        content: `@${teamAcronym} contra ${rivalAcronym} en el escenario... si fallan en fase de líneas o eligen el draft equivocado para el parche se acabó. ¡Duelo estelar para ${midPlayer}! 👊`,
        likes: 9340 + weekNum * 111,
        retweets: 4100 + weekNum * 39,
        timeAgo: 'hace 45 min',
        sentiment: 'positive' as const
...
export function RssFeedX({ tweets, isDark }: { tweets: any[]; isDark: boolean }) {
  const resolveTweetAvatar = (tweet: any) => {
    const influencer = tweet?.influencer || tweet;
    if (influencer?.avatar_blob) {
      return getImageUrl(influencer.avatar_blob);
    }
    // Check if the influencer has a custom logo/photo configured in db
    const handleId = influencer?.handle || '';
    const customUrl = getGameAssetUrl('influencers', handleId);
    ...
    return influencer?.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=80&q=80';
  };
        ...
        {[...tweets, ...tweets].map((tweet, idx) => (
          ...
          >
            <div className="flex items-center gap-2.5">
              <img 
                src={resolveTweetAvatar(tweet)} 
                alt="avatar" 
                referrerPolicy="no-referrer"
                className="w-7 h-7 rounded-full border border-cyan-500/20 shrink-0 object-cover bg-transparent" 
                onError={(e) => { (e.target as HTMLImageElement).src = 'assets/ui/fallback-influencer.png'; }}
              />
      ...
      <div className="absolute top-0 left-0 w-full animate-marquee-vertical-unified space-y-3.5 pr-0.5">
        {[...news, ...news].map((item, idx) => {
          ...
          const imageUrl = (itemAny.portal?.logo_blob)
            ...
            : getGameAssetUrl('press', item.outlet || item.portal_nome || item.id, 'assets/ui/fallback-press.png');
            ...
            >
              <div className="flex justify-between items-center gap-2">
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded overflow-hidden bg-transparent flex items-center justify-center border border-slate-700/10 dark:border-sky-500/15 shrink-0 p-0.5">
                    <img 
                      src={imageUrl} 
                      alt="Portal Logo" 
                      referrerPolicy="no-referrer"
