...
interface MarketTabProps {
  ...
  negotiateOnLoadPlayerId?: string | null;
  onClearNegotiateOnLoad?: () => void;
}
...
export default function MarketTab({
  ...
  const [extraFreeAgents, setExtraFreeAgents] = useState<Player[]>(() => GLOBAL_FREE_AGENTS);
  ...
  const handleOpenNegotiation = (player: Player, team: Team | null, isPre: boolean) => {
    ...
    // Check if player seeks contact / proposal with player contracted under another club outside pre-contract 6m window
    if (team && team.id !== gameState.playerTeamId && !isPre) {
      const nextState = { ...gameState };
      const teamInState = nextState.teams.find(t => t.id === gameState.playerTeamId);
      if (teamInState) {
        teamInState.budget = Math.max(0, teamInState.budget - 45000);
        teamInState.boardTrust = Math.max(1, teamInState.boardTrust - 15);
        teamInState.poachingPenaltiesWeeks = (teamInState.poachingPenaltiesWeeks || 0) + 2;
      }

      // Add social feed alert
      const poachingId = 'poaching_alert_' + Math.random().toString(36).substr(2, 9);
      nextState.socialFeed.unshift({
        ...
        content: `⚖️ [INFRAÇÃO DE ALICIAMENTO] A organização ${playerTeam.name} tentou contactar voluntariamente o atleta ${player.name} (${team.name}) fora do pré-contrato! Multa de $45.000 debitada pelo Compliance da Riot Games.`,
        likes: 3800,
        retweets: 980,
        timeAgo: 'Agora',
        sentiment: 'negative',
      ...
      triggerNotification?.(
        "⚖️ Multa de Poaching!",
        `Tentativa de aliciamento detectada! O clube foi multado em $45.000 pela Riot Compliance.`
      );

      alert(`⚖️ PROCESSO DE POACHING INICIADO!\n\nVocê tentou contactar diretamente o atleta ${player.name} sob contrato com a equipe ${team.name} (faltando ${player.contractMonths || 12} meses).\n\nConforme regulamento antitruste da liga, contatos fora dos últimos 6 meses de contrato são estritamente qualificados como Aliciamento (Poaching). Uma multa administrativa de $45.000 foi descontada do caixa da organização.`);
    }
  };

  useEffect(() => {
    if (negotiateOnLoadPlayerId) {
      const foundItem = unifiedDatabase.find(item => item.player.id === negotiateOnLoadPlayerId);
      if (foundItem) {
        handleOpenNegotiation(foundItem.player, foundItem.team, foundItem.isPreContract);
      }
      if (onClearNegotiateOnLoad) {
        onClearNegotiateOnLoad();
      }
    }
  }, [negotiateOnLoadPlayerId, unifiedDatabase, onClearNegotiateOnLoad]);
  ...
  return (
    <div className={`space-y-6 font-sans select-none min-h-screen ${
      isDark ? 'bg-[#0a1424] text-slate-100' : 'bg-slate-50 text-slate-800'
    }`}>
      ...
      <div className={`p-6 rounded-2xl border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${
        isDark ? 'bg-[#070d19] border-[#1e2d44]/75' : 'bg-white border-slate-200/80 shadow-xs'
      }`}>
        <div>
          ...
          <p className="text-[11px] text-slate-300 leading-snug mt-1 max-w-xl font-medium">
        ...
        <div className={`px-5 py-3 rounded-2xl border flex flex-col items-end shrink-0 ${
          isDark ? 'bg-[#070d19] border-[#1e2d44]/75' : 'bg-slate-50 border-slate-200 shadow-xs'
        }`}>
          ...
          <span className="text-[9px] text-slate-300 mt-1 font-semibold leading-none">
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all duration-150 cursor-pointer border ${
                  isActive
                    ...
                    : isDark ? 'bg-[#070d19] border-[#1e2d44]/60 text-slate-400 hover:text-white hover:bg-[#0a1424]' : 'bg-white hover:bg-slate-100 text-slate-600 border border-slate-200/60'
                }`}
              >
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className={`w-full py-2.5 pl-10 pr-4 text-xs font-semibold rounded-xl border focus:outline-none focus:ring-1 ${
              isDark 
                ? 'bg-[#070d19] border-[#1e2d44]/80 text-white focus:ring-cyan-500 focus:border-cyan-500' 
          ...
          <div className="relative">
            <select
              ...
              className={`w-full py-2 px-3 border rounded-xl text-xs font-bold appearance-none cursor-pointer focus:outline-none focus:ring-1 ${
                isDark ? 'bg-[#0a1424] border-[#1e2d44]/65 text-white focus:ring-cyan-500' : 'bg-white border-slate-200 text-slate-700 focus:ring-indigo-500'
              }`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 flex flex-col gap-4">
          {filteredOutput.length > 0 ? (
            <div className={`rounded-2xl border overflow-hidden ${
              isDark ? 'border-[#1e2d44]/40 bg-[#070d19]' : 'border-slate-200 bg-white shadow-xs'
            }`}>
              ...
              <div className={`hidden md:grid grid-cols-12 gap-1.5 px-4 py-3 text-[10px] font-mono font-black uppercase tracking-wider text-slate-400 border-b select-none ${
                isDark ? 'bg-[#0a1424] border-[#1e2d44]/75' : 'bg-slate-50 border-slate-200'
              }`}>
              ...
              {/* Items row iterator */}
              <div className={`divide-y ${isDark ? 'divide-[#1e2d44]/30' : 'divide-slate-200'}`}>
                  ...
                  return (
                    <div 
                      ...
                      className={`grid grid-cols-1 md:grid-cols-12 gap-1.5 px-4 py-4 items-center transition-all ${
                        isDark 
                          ? 'bg-[#0a1424]/40 hover:bg-[#1e2d44]/30' 
                    ...
                    >
                      ...
                      <div className="col-span-12 md:col-span-3 flex items-center gap-3">
                        ...
                        <div className="relative w-11 h-11 rounded-full overflow-hidden border border-slate-700/60 flex items-center justify-center shrink-0 bg-transparent">
                          {player.photoUrl || (player as any).face_blob ? (
                            <img 
                      ...
                      {/* Column 5: "Time" */}
                      <div className="col-span-12 md:col-span-2 flex flex-col justify-center items-start mt-1 md:mt-0 gap-1.5 min-w-0">
                        {team ? (
                          <div className="flex flex-col gap-1 w-full">
                            <div className="flex items-center gap-2 truncate">
                              {team.logoUrl || getGameAssetUrl('teams', team.id) ? (
                                <div className="w-5 h-5 rounded flex items-center justify-center shrink-0 overflow-hidden bg-slate-900/5 dark:bg-white/5 relative p-0.5">
                                  <img
                                    src={getGameAssetUrl('teams', team.id, team.logoUrl)}
                                    alt={team.name}
                              ...
                              <span className={`text-[#00E5FF] font-black tracking-wide truncate text-[11px] uppercase`}>
                                {team.acronym}
                              </span>
                            </div>
                            
                            {/* Proactive Contract highlight indicator */}
                            {isPreContract ? (
                              <span className="text-[8.5px] font-black text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20 uppercase tracking-widest leading-none">
                                🟢 [ 6 Meses Finais ]
                              </span>
                            ) : (
                              <span className="text-[8.5px] font-extrabold text-amber-500 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-550/25 uppercase tracking-wide leading-none" title="Contatos diretos de contratação resultam em penalidade por Aliciamento (Poaching)">
                                ⚠️ contrato: {player.contractMonths || 12}m
                              </span>
                            )}
                          </div>
                        ) : (
                      ...
                      <div className="col-span-12 md:col-span-2 text-left md:text-right flex md:flex-col justify-between items-baseline md:items-end mt-2 md:mt-0 pt-2 md:pt-0 border-t border-slate-800/10 md:border-0 md:pr-4">
                        ...
                        <span className="text-[8.5px] font-mono font-semibold text-slate-400 mt-1 block">
                          {formatMoney(Math.round((player.salary || player.overallRating * 1100 * 4) / 4))}/sem
                        </span>
                      </div>
                      ...
                      <div className="col-span-12 md:col-span-1 flex justify-end items-center gap-1.5 mt-3 md:mt-0">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-transform hover:scale-110 cursor-pointer flex items-center justify-center border shrink-0 ${
                            isDark 
                              ? 'bg-[#0a1424] border-[#1e2d44] hover:bg-[#1e2d44]/30 text-[#00E5FF]' 
                        ...
                        <button
                          ...
                          className={`w-9 h-9 rounded-full flex items-center justify-center cursor-pointer active:scale-90 transition-all shadow-md shrink-0 border ${
                            isDark
                              ? (isPreContract
                                  ? 'bg-rose-950/40 border-rose-800/50 hover:bg-rose-900/40 text-rose-450 font-bold shadow-none'
                                  : team
                                    ? 'bg-indigo-950/40 border-indigo-800/50 hover:bg-indigo-900/40 text-indigo-400 font-bold shadow-none'
                                    : 'bg-emerald-950/40 border-emerald-800/50 hover:bg-[#00FF88]/10 hover:border-[#00FF88]/30 text-emerald-400 font-bold shadow-none')
                              : (isPreContract
            ...
            <div className={`p-16 text-center border-2 border-dashed rounded-2xl ${
              isDark ? 'border-[#1e2d44]/50 bg-[#070d19] text-slate-400' : 'border-slate-200 bg-white text-slate-500'
            }`}>
              ...
              <p className="text-xs text-slate-300 mt-1 font-medium">Reajuste seus filtros táteis para prospectar novos candidatos.</p>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className={`p-5 rounded-2xl border ${
            isDark ? 'bg-[#070d19] border-[#1e2d44]' : 'bg-white border-slate-200 shadow-sm'
          }`}>
              ...
              <div className="space-y-4">
                {databaseWithObs.filter(x => x.isMonitored).map(({ player, team }) => (
                  <div key={player.id} className={`p-3.5 rounded-xl border flex flex-col gap-3 ${
                    isDark ? 'bg-[#0a1424]/80 border-[#1e2d44]/55' : 'bg-slate-50 border-slate-200'
                  }`}>
                    {/* Header */}
          ...
          <div className={`p-5 rounded-2xl border ${
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-300">Demanda p/ Suportes (CTRL)</span>
                <span className="text-emerald-500 font-extrabold">+24.5%</span>
              </div>
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-300">Valores de Franquias (LCK)</span>
                <span className="text-slate-350">Média $1.1M</span>
              </div>
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-300">Tempo Limite Registros</span>
          ...
          <div className={`p-5 rounded-2xl border ${
            isDark ? 'bg-[#070d19]/45 border-[#1e2d44]/60' : 'bg-slate-50 border-slate-200'
          }`}>
      ...
      {negotiatingPlayer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/60 select-none animate-fade-in">
          <div className={`w-full max-w-xl rounded-2xl border shadow-2xl relative overflow-hidden flex flex-col ${
            isDark ? 'bg-[#070d19] border-[#1e2d44] text-white' : 'bg-white border-slate-205 text-slate-800'
          }`}>
            
            {/* Modal Header */}
            <div className={`p-5 flex justify-between items-center border-b ${
              isDark ? 'bg-[#0a1424] border-[#1e2d44]' : 'bg-slate-50 border-slate-200'
            }`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className={`text-[8.5px] font-black tracking-widest uppercase block ${isDark ? 'text-cyan-405' : 'text-indigo-650'}`}>
            ...
            <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh]">
              ...
              <div className={`p-4 rounded-xl border flex gap-4 ${
                isDark ? 'bg-[#0a1424]/40 border-[#1e2d44]/50' : 'bg-slate-50 border-slate-200'
              }`}>
                <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 border border-slate-750 bg-transparent flex items-center justify-center">
                  <img 
                    src={getGameAssetUrl('players', negotiatingPlayer.id, negotiatingPlayer.photoUrl)} 
                    className="w-full h-full object-contain bg-transparent" 
                    referrerPolicy="no-referrer" 
                ...
                <div>
                  ...
                  <p className="text-[10.5px] text-sky-400 mt-1 font-bold flex flex-col gap-2 leading-tight">
                    {isPreContrato ? (
                      <span className="text-emerald-400 font-extrabold flex items-center gap-1">
                        🟢 [ 6 MESES FINAIS ] PRÉ-CONTRATO VÁLIDO: Passe gratuito! Não infringe as regras de Aliciamento (Poaching). O atleta migrará ao término do split atual.
                      </span>
                    ) : negotiatingTeam ? (
                      <span className="flex flex-col gap-1.5">
                        <span className="flex items-center gap-1.5 flex-wrap text-slate-350">
                          Pertence ao clube: 
                          {isPreContrato ? null : negotiatingTeam?.logoUrl || getGameAssetUrl('teams', negotiatingTeam?.id) ? (
                            <img 
                              src={getGameAssetUrl('teams', negotiatingTeam.id, negotiatingTeam.logoUrl)} 
                        ...
                        <span className="text-red-405 font-black bg-red-500/10 border border-red-500/20 p-2 rounded text-[10px] leading-relaxed block uppercase tracking-wide">
                          ⚠️ ALERTA DE ALICIAMENTO DA LIGA (!): O jogador possui {negotiatingPlayer.contractMonths || 12} meses de vínculo restante. Abordagem fora do pré-contrato acarreta multa imediata de de US$ 45.000 + punições disciplinares!
                        </span>
                      </span>
                    ) : (
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className={`w-full py-2.5 pl-8 pr-4 text-xs font-bold rounded-xl border focus:outline-none focus:ring-1 ${
                            isDark ? 'bg-[#0a1424] border-[#1e2d44] text-white focus:ring-cyan-500' : 'bg-white border-slate-250 text-slate-800 focus:ring-indigo-505'
                          }`}
                        />
                      </div>

                      {/* Shortcuts */}
                      <div className="flex gap-2 text-[9px] font-mono font-black text-slate-300 leading-none">
                        <button
                          onClick={() => setOfferTransferValue(negotiatingPlayer.marketValue)}
                          className={`px-2.5 py-1.5 rounded cursor-pointer ${isDark ? 'bg-[#1a283e] border border-[#1e2d44] hover:bg-[#1e2d44]' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}
                        >
                          [Mercado]
                        </button>
                        <button
            ...
            {negotiationState === 'idle' && (
              <div className={`p-5 flex justify-end gap-3 border-t select-none ${
                isDark ? 'bg-[#0a1424]/70 border-[#1e2d44]' : 'bg-slate-55 border-slate-200'
              }`}>
                <button
                  onClick={() => setNegotiatingPlayer(null)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${
                    isDark ? 'bg-slate-900 border border-[#1e2d44] text-slate-300 hover:bg-slate-800' : 'bg-white border border-slate-250 text-slate-705 hover:bg-slate-50'
                  }`}
                >
                  Desistir
                </button>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 backdrop-blur-md bg-black/60 select-none animate-fade-in">
          <div className={`w-full max-w-md rounded-2xl border shadow-2xl relative overflow-hidden flex flex-col ${
            ...
            <div className={`p-4 flex justify-between items-center border-b ${
              isDark ? 'bg-[#0a1424] border-[#1e2d44]' : 'bg-slate-50 border-slate-205'
            }`}>
            ...
            <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
              ...
              <div className={`p-3 rounded-xl border flex gap-3 items-center ${
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 border border-slate-700/50 bg-transparent flex items-center justify-center">
                  <img 
                    src={getGameAssetUrl('players', allocatingPlayer.id, allocatingPlayer.photoUrl)} 
                    className="w-full h-full object-contain bg-transparent" 
                    referrerPolicy="no-referrer" 
                    alt={allocatingPlayer.name} 
                    onError={(e) => { e.currentTarget.src = 'assets/ui/fallback-player.png'; }}
                  />
                </div>
                <div>
                      ...
                      <button
                        ...
                        className={`text-[9px] font-mono font-black px-2.5 py-1.5 rounded-lg uppercase cursor-pointer active:scale-95 transition-all text-center ${
                          isAssignedToThis
                            ...
                            : 'bg-cyan-500 hover:bg-cyan-400 text-slate-950 shadow-md shadow-cyan-455/10'
                        }`}
                      >
            ...
            <div className={`p-4 flex justify-end border-t ${
              isDark ? 'bg-[#0a1424]/45 border-[#1e2d44]' : 'bg-slate-50 border-slate-200'
            }`}>
              <button
                onClick={() => setAllocatingPlayer(null)}
                className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${
                  isDark ? 'bg-slate-900 border border-[#1e2d44] text-slate-300 hover:bg-slate-800' : 'bg-white border border-slate-250 text-slate-700 hover:bg-slate-50'
                }`}
              >
                Voltar ao Mercado
              </button>
