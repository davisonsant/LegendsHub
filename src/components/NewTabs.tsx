...
import { 
  ...
  Percent, Lock, Scale, Clock, AlertTriangle, MapPin, Star, StarHalf, Globe, Terminal, RefreshCw, Search
} from 'lucide-react';
import { GameState, Team, Player, Sponsor, Champion, MatchSeries, Position, CorporationStaff, Manager } from '../types';
...
import { getGameAssetUrl, getImageUrl, getProceduralFallbackUrl } from '../utils/gameAssets';
...
interface LigaTabProps {
  gameState: GameState;
  theme?: 'light' | 'dark';
  initialActiveTab?: 'Torneios' | 'Classificação' | 'Calendário' | 'Regras' | 'Jogos';
  onResetInitialTab?: () => void;
}
...
const TOURNAMENTS: Tournament[] = [
  ...
  },
  {
    ...
    format: '14 times, turno único Bo3, G10 King-of-the-Hill playoffs.',
    rewards: [
  ...
  },
  {
    ...
    format: '14 times, turno único Bo3, os 3 primeiros colocados garantem vaga no CBOLÃO.',
    rewards: [
      { rank: '1º VAGA CBOLÃO', value: 'Classificado', style: 'bg-purple-500/15 border-purple-500 text-purple-400' },
      { rank: '2º VAGA CBOLÃO', value: 'Classificado', style: 'bg-purple-500/15 border-purple-500 text-purple-400' },
      { rank: '3º VAGA CBOLÃO', value: 'Classificado', style: 'bg-purple-500/15 border-purple-500 text-purple-400' },
...
export function LigaTab({ gameState, theme, initialActiveTab, onResetInitialTab }: LigaTabProps) {
  ...
  useEffect(() => {
    if (initialActiveTab) {
      ...
      if (onResetInitialTab) {
        onResetInitialTab();
      }
    }
  }, [initialActiveTab, onResetInitialTab]);
  const [selectedTourneyId, setSelectedTourneyId] = useState<string>(playerRegion);

  const handleTimelineClick = (tourneyId: string) => {
    setSelectedTourneyId(tourneyId);
    setActiveTab('Torneios');
  };

  const getAcademyTourneyId = () => {
    if (playerRegion === 'LEC') return 'Youth Academy LEC';
    if (playerRegion === 'LCK') return 'Youth Academy LCK';
    if (playerRegion === 'LPL') return 'Youth Academy LPL';
    if (playerRegion === 'LCS') return 'Youth Academy LCS';
    return 'DESAFIANTE';
  };
  ...
  return (
    <div className="space-y-6 select-none font-sans">
      ...
      {/* 1. SECTOR HEADER */}
      <div className={`${isDark ? 'bg-[#0a1424] border-2 border-[#1e2d44] text-white' : 'bg-white border-2 border-slate-800/10 text-slate-850'} p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={`font-display text-lg font-black uppercase tracking-wider flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
              <span>LIGA: {selectedTourney.id.toUpperCase() === 'WORLDS' ? 'WORLDS' : selectedTourney.id}</span>
            </h1>
            <p className={`text-[11px] font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              {(() => {
                const count = standings.length;
                if (selectedTourney.id === 'MSI') {
                  return `Torneio Mundial Intermediário • Mundial • ${count} franquias participantes`;
                }
                if (selectedTourney.id === 'Worlds') {
                  return `Torneio Mundial • Mundial de League of Esports • ${count} franquias participantes`;
                }
                if (selectedTourney.category === 'Base') {
                  return `Liga Regional de Desenvolvimento • ${selectedTourney.country} • ${count} franquias participantes`;
                }
                return `Liga Regional • ${selectedTourney.country} • ${count} franquias participantes`;
              })()}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-3.5">
        <div className={`flex flex-wrap items-center justify-between gap-3 border-b pb-3 ${
          isDark ? 'border-[#1e2d44]/60' : 'border-slate-800/15'
        }`}>
          ...
          <div className="flex flex-wrap gap-2.5">
            ...
            <div className={`flex items-center gap-0.5 p-1 rounded-lg text-[10px] ${
              isDark ? 'bg-slate-900/60' : 'bg-slate-100'
            }`}>
              <button
                ...
                className={`px-2.5 py-1 rounded font-bold uppercase transition-all cursor-pointer ${
                  filterMyTourneys === 'meus'
                    ? (isDark ? 'bg-slate-800 text-[#00cbd6] shadow-sm font-black' : 'bg-white text-[#00cbd6] shadow-sm font-black')
                    : 'text-slate-500'
                }`}
              >
                Meus Torneios
            ...
            <div className={`flex items-center gap-0.5 p-1 rounded-lg text-[10px] ${
              ...
              {(['all', 'regionais', 'internacionais'] as const).map((tFilter) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded font-bold uppercase transition-all cursor-pointer ${
                    filterType === tFilter
                      ? (isDark ? 'bg-slate-800 text-cyan-500 shadow-sm font-black' : 'bg-white text-cyan-500 shadow-sm font-black')
                      : 'text-slate-500'
                  }`}
                >
      ...
      <div className="animate-fade-in">
        ...
        {activeTab === 'Torneios' && (
          <div className="space-y-4">
            <p className={`text-xs ${s.textMuted} font-semibold uppercase tracking-wider`}>
              Inspecione todas as competições e ligas registradas:
            </p>
                    ...
                    <div className="col-span-4 flex items-center gap-3">
                      <div className="w-1.5 h-6 rounded-full shrink-0" style={{ backgroundColor: team.primaryColor || '#00cbd6' }} />
                       ...
                       <div className="w-6 h-6 rounded flex items-center justify-center shrink-0 overflow-hidden relative"
                         ...
                         <img 
                           src={getImageUrl(team?.logo_blob) || getGameAssetUrl('teams', team?.id?.replace('-academy', ''), team?.logoUrl)} 
                           alt={team?.name} 
            ...
            <div className="mt-8 space-y-6 animate-fade-in text-left">
              <div className={`p-6 rounded-2xl ${isDark ? 'bg-[#0a1424] border border-[#1e2d44]' : 'bg-white border border-slate-200 shadow-sm'} shadow-xl border-purple-500/30 max-w-full overflow-hidden`}>
                <div className={`flex items-center gap-3 border-b pb-3 mb-6 ${isDark ? 'border-purple-500/20' : 'border-purple-500/10'}`}>
                  ...
                  <div>
                    <h3 className={`text-sm font-black uppercase tracking-wider ${isDark ? 'text-purple-400' : 'text-purple-700'}`}>Chaveamento do CBOLÃO (Playoffs de 15 Equipes)</h3>
                    <p className={`text-[10px] leading-relaxed ${isDark ? 'text-zinc-300' : 'text-slate-650'}`}>
                      Os participantes do CBOLÃO são os top 3 academy (de base) de cada liga (região) para confrontos todos contra todos na fase classificatória (Tabela Classificação). O primeiro colocado da tabela é classificado diretamente para a final do bracket (Playoffs).
                    </p>
                  </div>
                </div>
                    ...
                    {Array.from({ length: 4 }).map((_, i) => (
                      <div key={i} className="h-28 flex items-center justify-center">
                        <svg className="w-full h-full" viewBox="0 0 24 100" fill="none" preserveAspectRatio="none">
                          <path d="M 0 25 L 12 25 L 12 75 L 24 75 M 12 50 L 24 50" stroke={isDark ? "rgb(168 85 247 / 0.5)" : "rgb(168 85 247 / 0.3)"} strokeWidth="2" strokeLinecap="round" />
                        </svg>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex-1 min-w-[240px] flex flex-col justify-center space-y-4">
                    ...
                    <div className={`p-5 rounded-2xl border-2 border-purple-500 shadow-2xl relative space-y-4 ${
                      ...
                      <div className="pt-2 text-center border-t border-purple-500/15">
                        <span className={`text-[7.5px] font-mono tracking-widest uppercase font-black block mb-1 ${isDark ? 'text-[#94a3b8]' : 'text-[#475569]'}`}>CAMPEÃO</span>
          ...
          <div className={`${s.bgCard} p-6 rounded-2xl shadow-xl space-y-6 border border-slate-800/10 dark:border-[#1e2d44]/40`}>
            ...
            <div className="overflow-x-auto pb-4 scrollbar-none" style={{ scrollbarWidth: 'none' }}>
              <div className="min-w-[780px] space-y-4 relative pt-1.5">
                ...
                {/* Track Columns MONTHS list indicator */}
                <div className="grid gap-0.5 text-[8px] font-bold text-center uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-dashed border-slate-800/10 dark:border-[#1e2d44]/20 pb-1 font-sans" style={{ gridTemplateColumns: 'repeat(45, minmax(0, 1fr))' }}>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Janeiro</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Fevereiro</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Março</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Abril</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Maio</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Junho</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Julho</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Agosto</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Setembro</div>
                  <div className="border-r border-slate-800/5 dark:border-slate-800/20" style={{ gridColumn: 'span 4 / span 4' }}>Outubro</div>
                  <div style={{ gridColumn: 'span 5 / span 5' }}>Novembro</div>
                </div>

                {/* Track Columns Week list indicator */}
                <div className="grid gap-0.5 border-b border-slate-800/10 dark:border-slate-800/30 pb-2 text-[8px] font-mono text-slate-400 font-black" style={{ gridTemplateColumns: 'repeat(45, minmax(0, 1fr))' }}>
                ...
                <div className="space-y-1 relative">
                  <div className="text-[8.5px] font-black uppercase text-slate-400 tracking-wider font-mono">
                    CIRCUITO REGIONA DE ELITES: (LCK, LPL, LEC, CBLOL, LCS, LCP)
                  </div>
                  <div className="grid gap-0.5 h-6 relative bg-slate-900/10 dark:bg-slate-900/30 rounded-md overflow-hidden" style={{ gridTemplateColumns: 'repeat(45, minmax(0, 1fr))' }}>
                    <button 
                      onClick={() => handleTimelineClick(playerRegion)}
                      className="absolute top-0 bottom-0 left-0 w-[8.88%] bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/30 rounded flex items-center justify-center text-[7.5px] font-bold text-yellow-500 transition-all cursor-pointer select-none focus:outline-none"
                    >
                      OFFSEASON
                    </button>
                    <button 
                      onClick={() => handleTimelineClick(playerRegion)}
                      className="absolute top-0 bottom-0 left-[8.88%] w-[26.6%] bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-500/30 rounded flex items-center justify-center text-[7.5px] font-bold text-cyan-400 transition-all cursor-pointer select-none"
                    >
                      SPRING
                    </button>
                    <button 
                      onClick={() => handleTimelineClick('MSI')}
                      className="absolute top-0 bottom-0 left-[35.5%] w-[8.88%] bg-pink-500/15 hover:bg-pink-500/25 border border-pink-500/30 rounded flex items-center justify-center text-[7.5px] font-bold text-pink-400 transition-all cursor-pointer select-none font-black"
                    >
                      MSI
                    </button>
                    <button 
                      onClick={() => handleTimelineClick(playerRegion)}
                      className="absolute top-0 bottom-0 left-[44.38%] w-[26.6%] bg-blue-500/15 hover:bg-blue-500/25 border border-blue-500/30 rounded flex items-center justify-center text-[7.5px] font-bold text-blue-400 transition-all cursor-pointer select-none"
                    >
                      SUMMER
                    </button>
                    <button 
                      onClick={() => handleTimelineClick(playerRegion)}
                      className="absolute top-0 bottom-0 left-[71%] w-[13.3%] bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 rounded flex items-center justify-center text-[7.5px] font-bold text-red-500 transition-all cursor-pointer select-none"
                    >
                      PLAYOFFS
                    </button>
                  </div>
                ...
                <div className="space-y-1 relative">
                  <div className="text-[8.5px] font-black uppercase text-slate-400 tracking-wider font-mono">
                    Divisões de Base & Academy (Desafiante, Youth Academy LEC/LCK/LPL/LCS)
                  </div>
                  <div className="grid gap-0.5 h-6 relative bg-slate-900/10 dark:bg-slate-900/30 rounded-md overflow-hidden" style={{ gridTemplateColumns: 'repeat(45, minmax(0, 1fr))' }}>
                    <button 
                      onClick={() => handleTimelineClick(getAcademyTourneyId())}
                      className="absolute top-0 bottom-0 left-0 w-[35.5%] bg-green-500/10 hover:bg-green-500/20 border border-green-500/20 rounded flex items-center justify-center text-[7.5px] font-bold text-green-400 transition-all cursor-pointer select-none"
                    >
                      STAGE 1 ACADEMY
                    </button>
                    <button 
                      onClick={() => handleTimelineClick(getAcademyTourneyId())}
                      className="absolute top-0 bottom-0 left-[35.5%] w-[8.88%] bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/20 rounded flex items-center justify-center text-[7.5px] font-bold text-teal-400 transition-all cursor-pointer select-none"
                    >
                      SCOUTING TRIALS
                    </button>
                    <button 
                      onClick={() => handleTimelineClick(getAcademyTourneyId())}
                      className="absolute top-0 bottom-0 left-[44.38%] w-[39.9%] bg-green-500/15 hover:bg-green-500/25 border border-green-500/25 rounded flex items-center justify-center text-[7.5px] font-bold text-green-400 transition-all cursor-pointer select-none"
                    >
                ...
                <div className="space-y-1 relative">
                  ...
                  <div className="grid gap-0.5 h-6 relative bg-slate-900/10 dark:bg-slate-900/30 rounded-md overflow-hidden" style={{ gridTemplateColumns: 'repeat(45, minmax(0, 1fr))' }}>
                    <button 
                      onClick={() => handleTimelineClick('MSI')}
                      className="absolute top-0 bottom-0 left-[31.1%] w-[13.3%] bg-pink-500/20 hover:bg-pink-500/30 border border-pink-500/40 rounded flex items-center justify-center text-[7.5px] font-bold text-pink-500 transition-all cursor-pointer select-none"
                    >
                      👑 MSI GLOBAL
                    </button>
                    <button 
                      onClick={() => handleTimelineClick('CBOLÃO')}
                      className="absolute top-0 bottom-0 left-[66.6%] w-[6.6%] bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/40 rounded flex items-center justify-center text-[7.5px] font-bold text-purple-400 transition-all cursor-pointer select-none"
                    >
                      🧅 CBOLÃO
                    </button>
                  </div>
                ...
                <div className="space-y-1 relative">
                  ...
                  <div className="grid gap-0.5 h-6 relative bg-slate-900/10 dark:bg-slate-900/30 rounded-md overflow-hidden" style={{ gridTemplateColumns: 'repeat(45, minmax(0, 1fr))' }}>
                    <button 
                      onClick={() => handleTimelineClick('Worlds')}
                      className="absolute top-0 bottom-0 left-[84.4%] w-[13.3%] bg-amber-500/30 hover:bg-amber-500/40 border border-amber-500/60 rounded flex items-center justify-center text-[7.5px] font-extrabold text-amber-500 transition-all cursor-pointer select-none"
                    >
                      🔥 WORLDS (MUNDIAL)
                    </button>
                  </div>
        ...
        {activeTab === 'Regras' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className={`${s.bgCard} p-5 rounded-2xl border border-slate-800/10 dark:border-[#1e2d44]/40 space-y-4`}>
                <div className="flex items-center gap-2 border-b border-slate-800/10 dark:border-[#1e2d44]/40 pb-2">
                  ...
                  <h4 className={`text-xs font-black uppercase tracking-wider ${isDark ? 'text-white' : 'text-slate-800'}`}>FORMATO DA COMPETIÇÃO</h4>
                </div>
              ...
              <div className={`${s.bgCard} p-5 rounded-2xl border border-slate-800/10 dark:border-[#1e2d44]/40 space-y-4`}>
                <div className="flex items-center gap-2 border-b border-slate-800/10 dark:border-[#1e2d44]/40 pb-2">
                  ...
                  <h4 className={`text-xs font-black uppercase tracking-wider ${isDark ? 'text-white' : 'text-slate-800'}`}>CLASSIFICAÇÃO E VAGAS</h4>
                </div>
                <p className="text-[10.5px] text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-black uppercase tracking-widest text-[#00cbd6]">PREMIAÇÃO</h3>
...
const getCustomLeagueData = (leagueId: string, gameState: GameState) => {
  const assetLogo = getGameAssetUrl('leagues', leagueId);
  if (assetLogo && assetLogo !== '/assets/ui/fallback-silhouette.png' && assetLogo !== '/perfil-default.jpeg') {
    if (gameState && gameState.leagues) {
      const match = gameState.leagues.find(lg => lg.id === leagueId);
      return { name: match ? match.name : leagueId, logo: assetLogo };
    }
    try {
      const saved = localStorage.getItem('legendshub_custom_leagues_meta');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed && parsed[leagueId]) {
          return { name: parsed[leagueId].name || leagueId, logo: assetLogo };
        }
      }
    } catch(e) {}
    return { name: leagueId, logo: assetLogo };
  }

  if (gameState && gameState.leagues) {
    const match = gameState.leagues.find(lg => lg.id === leagueId);
    if (match && match.logoUrl) {
      return { name: match.name || leagueId, logo: match.logoUrl };
    }
  }
  try {
    const saved = localStorage.getItem('legendshub_custom_leagues_meta');
    if (saved) {
      ...
      if (parsed && parsed[leagueId]) {
        return {
          name: parsed[leagueId].name || leagueId,
          logo: parsed[leagueId].logo || parsed[leagueId].logoUrl || ''
        };
      }
    }
  } catch (e) {}
  return { name: leagueId, logo: '' };
};

const renderInsigniaFallback = (leagueId: string, sizeClass: string) => {
  let gradient = 'from-emerald-600 via-teal-500 to-yellow-500';
  let icon = <Shield className="w-5 h-5 text-yellow-100" />;
  
  if (leagueId === 'LCK') {
    gradient = 'from-blue-700 via-indigo-650 to-purple-650';
    icon = <Crown className="w-5 h-5 text-blue-100" />;
  } else if (leagueId === 'LPL') {
    gradient = 'from-rose-750 via-red-650 to-amber-500';
    icon = <Zap className="w-5 h-5 text-amber-100" />;
  } else if (leagueId === 'LEC') {
    gradient = 'from-pink-600 via-purple-650 to-indigo-500';
    icon = <Award className="w-5 h-5 text-pink-100" />;
  } else if (leagueId === 'LCS') {
    gradient = 'from-cyan-600 via-sky-500 to-indigo-600';
    icon = <Trophy className="w-5 h-5 text-cyan-100" />;
  } else if (leagueId === 'LCP') {
    gradient = 'from-fuchsia-600 via-purple-600 to-pink-500';
    icon = <Sparkles className="w-5 h-5 text-fuchsia-100" />;
  }

  return (
    <div className={`${sizeClass} rounded-xl bg-gradient-to-br ${gradient} p-[1.5px] shadow-lg flex items-center justify-center shrink-0`}>
      <div className="w-full h-full rounded-[10.5px] bg-[#070d19]/85 backdrop-blur-sm flex items-center justify-center transition-all duration-300 group-hover:bg-[#070d19]/55">
        {icon}
      </div>
    </div>
  );
...
const renderLeagueInsignia = (leagueId: string, customLogoUrl?: string, sizeClass: string = "w-12 h-12") => {
  if (customLogoUrl && customLogoUrl.trim() !== '') {
    return (
      <div className={`${sizeClass} shrink-0 relative flex items-center justify-center`}>
        <img 
          src={customLogoUrl} 
          alt={leagueId} 
          className="w-full h-full rounded-xl object-contain border border-slate-200/20 bg-slate-900/50 p-1 shadow-inner"
          referrerPolicy="no-referrer"
          onError={(e) => {
        ...
        />
        <div className="absolute inset-0 hidden flex-col items-center justify-center rounded-xl bg-[#070d19]">
          {renderInsigniaFallback(leagueId, sizeClass)}
        </div>
      </div>
    );
  }
  return renderInsigniaFallback(leagueId, sizeClass);
};
...
export function TimesTab({ 
  ...
  const [selectedRegionId, setSelectedRegionId] = useState<string | null>(initialSelectedRegionId || null);
      ...
      <div className="space-y-6 select-none font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {LEAGUE_DATABASE.map((league) => {
            ...
            const customData = getCustomLeagueData(league.id, gameState);
            const leagueName = customData.name || league.name;
            const leagueLogo = customData.logo;

            return (
              <button
                key={league.id}
                onClick={() => setSelectedRegionId(league.id)}
                className={`relative overflow-hidden rounded-2xl text-left flex flex-col justify-between h-56 transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-cyan-505/50 group cursor-pointer border ${
                  isDark
                    ? 'border-[#1e2d44]/75 bg-[#030712] shadow-[0_4px_20px_rgba(0,0,0,0.4)] hover:border-cyan-500 hover:shadow-[0_0_25px_rgba(0,210,253,0.12)]'
                    : 'border-slate-300 bg-[#050b18] shadow-md hover:border-blue-500 hover:shadow-xl text-white'
                }`}
              >
                {/* Content Overlay */}
                <div className="relative p-5 z-10 w-full flex flex-col justify-between h-full">
                  <div className="flex justify-between items-start">
                    {/* Badge Regional */}
                    <span className="bg-sky-500/15 border border-sky-455/40 text-sky-400 text-[9px] font-black uppercase px-2.5 py-1 rounded-md tracking-wider">
                      {regionTag}
                    </span>
                  ...
                  <div className="flex items-center gap-3.5 my-3">
                    {renderLeagueInsignia(league.id, leagueLogo, "w-11 h-11")}
                    <div className="truncate">
                      <h3 className="font-display font-black text-sm text-white tracking-wider flex items-center gap-1 group-hover:text-cyan-400 transition-colors uppercase">
                        {leagueName} <ChevronRight className="w-4 h-4 text-cyan-400 group-hover:translate-x-1.5 duration-200 shrink-0" />
                      </h3>
  ...
  if (selectedRegionId && !selectedTeamId) {
    ...
    return (
      <div className="space-y-6 select-none font-sans">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-between sm:items-center border-b pb-4 border-slate-200/20 dark:border-[#1e2d44]/40">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <h3 className={`font-display text-xs font-black uppercase tracking-wider flex items-center gap-1.5 ${s.textWhiteOrSlate}`}>
              Ligas / <span className="text-cyan-400">{selectedRegionId && getCustomLeagueData(selectedRegionId, gameState).name} ({activeRegion?.country})</span>
            </h3>
          </div>
          ...
          <div className="flex items-center gap-1 p-1 bg-slate-950/40 dark:bg-black/30 rounded-lg max-w-sm border border-slate-200/5 dark:border-[#1e2d44]/25">
            {(['roster', 'academy'] as const).map(div => (
              ...
              >
                {div === 'roster' ? 'TIME PRINCIPAL' : 'ACADEMY'}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Small League Banner */}
        {activeRegion && (() => {
          const customData = getCustomLeagueData(activeRegion.id, gameState);
          const leagueName = customData.name || activeRegion.name;
          const leagueLogo = customData.logo;
          return (
            <div className={`relative overflow-hidden rounded-xl border flex items-center gap-4 p-5 ${
              isDark ? 'bg-[#030712] border-[#1e2d44]/55' : 'bg-slate-900 border-slate-750'
            }`}>
              {renderLeagueInsignia(activeRegion.id, leagueLogo, "w-12 h-12")}
              <div className="relative z-10 space-y-1 min-w-0 flex-1">
                <h2 className="text-sm sm:text-base font-black font-display tracking-wider text-white uppercase font-bold">
                  {(() => {
                    const upper = activeRegion.id.toUpperCase();
                    switch (upper) {
                      case 'LCK': return 'League of Legends Champions Korea';
                      case 'CBLOL': return 'Campeonato Brasileiro de League of Legends';
                      case 'LPL': return 'League of Legends Pro League';
                      case 'LEC': return 'League of Legends EMEA Championship';
                      case 'LCS': return 'League Championship Series';
                      case 'LCP': return 'League of Legends Championship Pacific';
                      default: return `${leagueName} Professional Circuit`;
                    }
                  })()}
                </h2>
              ...
              >
                ...
                <div className="flex items-start justify-between w-full">
                  <div className="flex items-center gap-2.5 truncate">
                    ...
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 overflow-hidden relative"
                         style={{ backgroundColor: 'transparent' }}>
                      <img 
                        src={getImageUrl(t?.logo_blob) || getGameAssetUrl('teams', t?.id?.replace('-academy', ''), t?.logoUrl)} 
                        alt={t?.name} 
                        className="w-full h-full object-contain p-0.5 rounded"
                        referrerPolicy="no-referrer"
                        onError={(e) => { e.currentTarget.src = 'assets/ui/fallback-logo.png'; }}
                ...
                <div className="space-y-1.5 border-t border-slate-200/10 dark:border-[#1e2d44]/30 pt-3 text-[10px] w-full">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className={`p-1 px-1.5 rounded border text-center ${
                      isDark ? 'bg-slate-950/60 border-slate-800 text-slate-200' : 'bg-slate-100/50 border-slate-200 text-slate-800'
                    }`}>
      ...
      <div className="space-y-6 select-none font-sans">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-between sm:items-center border-b pb-4 border-slate-200/20 dark:border-[#1e2d44]/40">
          ...
          <div className="flex items-center gap-2">
            ...
            {activeLeague && (() => {
              const customData = getCustomLeagueData(activeLeague.id, gameState);
              const customLogo = customData.logo;
              const nameToUse = customData.name || activeLeague.name;
              return (
                <div className="flex items-center gap-1.5 bg-black/40 text-sky-450 px-2.5 py-1 rounded text-[10px] font-bold border border-sky-400/20 backdrop-blur-sm">
                  {customLogo ? (
                    <img src={customLogo} alt={nameToUse} className="w-4 h-4 rounded object-contain border border-[#00cbd6]/30" referrerPolicy="no-referrer" />
                  ) : (
                    <Shield className="w-3.5 h-3.5 text-cyan-400" />
                  )}
                  <span>{nameToUse} ({activeRegion?.country})</span>
                </div>
              );
            })()}
          </div>
        ...
        >
          ...
          <div className="flex items-center gap-5 z-10">
            <div 
              className="w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center shadow-lg border border-white/20 bg-transparent"
              style={{ backgroundColor: activeTeam.primaryColor }}
            >
              <img 
                src={getImageUrl(activeTeam?.logo_blob) || getGameAssetUrl('teams', activeTeam.id, getProceduralFallbackUrl(activeTeam.name, true))} 
                onError={(e) => { (e.target as HTMLImageElement).src = 'assets/ui/fallback-logo.png'; }} 
                className="bg-transparent object-contain w-12 h-12"
                alt={activeTeam.name}
              />
            </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className={`p-5 rounded-xl border ${s.bgCard} shadow-sm space-y-3`}>
                <div className="flex justify-between items-center">
                  <span className={`text-[10px] uppercase font-extrabold tracking-wider ${isDark ? 'text-slate-405' : 'text-[#0B1B3D]'}`}>Verba de Caixa</span>
                  <DollarSign className="w-4 h-4 text-emerald-450" />
                </div>
                <div className="space-y-1">
                  <h3 className={`text-xl font-black font-mono tracking-tight ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>
                    $ {activeTeam.budget.toLocaleString('pt-BR')}
                  </h3>
                  <span className={`block text-[9.5px] font-bold ${activeTeam.budget > 1500000 ? (isDark ? 'text-emerald-400' : 'text-emerald-700') : (isDark ? 'text-amber-400' : 'text-amber-700')}`}>
                    {activeTeam.budget > 1500000 ? '● Liquidez de Transferências Alta' : '● Rigor Financeiro Recomendado'}
                  </span>
                </div>
                <p className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'} leading-relaxed`}>
              ...
              <div className={`p-5 rounded-xl border ${s.bgCard} shadow-sm space-y-3`}>
                <div className="flex justify-between items-center">
                  <span className={`text-[10px] uppercase font-extrabold tracking-wider ${isDark ? 'text-slate-405' : 'text-[#0B1B3D]'}`}>ENGAJAMENTO DE TORCIDA</span>
                ...
                <div className="space-y-1">
                  ...
                  <span className={`block text-[9.5px] font-bold ${isDark ? 'text-sky-400' : 'text-sky-700'}`}>
            ...
            <div className={`p-5 rounded-xl border ${s.bgCard} shadow-sm space-y-4`}>
              <div className="flex items-center gap-2 border-b border-slate-200/10 dark:border-[#1e2d44]/30 pb-2.5">
                <Building2 className={`w-4 h-4 ${isDark ? 'text-sky-450' : 'text-[#0B1B3D]'}`} />
                <h4 className={`text-xs font-black uppercase tracking-wider ${isDark ? 'text-white' : 'text-[#0B1B3D]'}`}>ESTRUTURA DE GAMING HOUSE & CT</h4>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className={`p-3.5 rounded-lg ${s.bgInnerCard} space-y-1.5`}>
                  <span className={`block text-[9px] uppercase font-bold ${isDark ? 'text-slate-450' : 'text-[#0B1B3D]'}`}>ESTRUTURA DE GAMING HOUSE</span>
                  <div className="flex items-center gap-1.5">
                    <span className={`text-base font-black font-mono ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>Lvl {activeTeam.infrastructure?.gamingHouseLevel || 1}</span>
                  ...
                  <p className={`text-[9.5px] ${isDark ? 'text-slate-400' : 'text-slate-650'}`}>Infraestrutura residencial de alojamento para atletas e staff.</p>
                </div>

                <div className={`p-3.5 rounded-lg ${s.bgInnerCard} space-y-1.5`}>
                  <span className={`block text-[9px] uppercase font-bold ${isDark ? 'text-slate-450' : 'text-[#0B1B3D]'}`}>Centro de Treinamento (CT)</span>
                  <div className="flex items-center gap-1.5">
                    <span className={`text-base font-black font-mono ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>Lvl {activeTeam.infrastructure?.trainingCenterLevel || 1}</span>
                    <div className="flex gap-0.5">
                      {Array.from({ length: 5 }).map((_, idx) => (
                        <span 
                          key={idx} 
                  ...
                  <p className={`text-[9.5px] ${isDark ? 'text-slate-400' : 'text-slate-650'}`}>Salas de scrims dedicadas, simulação tecnológica e hardware.</p>
                </div>
              </div>
            ...
            <div className={`p-5 rounded-xl border ${s.bgCard} shadow-sm space-y-3.5`}>
              <div className="flex items-center justify-between border-b border-slate-200/10 dark:border-[#1e2d44]/30 pb-2.5">
                <div className="flex items-center gap-2">
                  <Award className={`w-4 h-4 ${isDark ? 'text-amber-400' : 'text-[#0B1B3D]'}`} />
                  <h4 className={`text-xs font-black uppercase tracking-wider ${isDark ? 'text-white' : 'text-[#0B1B3D]'}`}>Patrocinador Principal Ativo</h4>
                </div>
                <span className={`text-[9.5px] ${isDark ? 'text-emerald-400' : 'text-emerald-700'} font-extrabold uppercase`}>Contrato Ativo</span>
              </div>

              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-lg ${isDark ? 'bg-slate-550/10 border-slate-205/10 text-white' : 'bg-slate-100 border-slate-305 text-slate-800'} flex items-center justify-center font-bold text-center text-[10px] break-words uppercase p-1`}>
                  {sponsorName.substring(0, 5)}
                </div>
                <div className="flex-1 space-y-1">
                  <h5 className={`text-xs font-black uppercase ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>{sponsorName}</h5>
                  <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
                    <div>
                      <span className={`text-[8px] ${isDark ? 'text-slate-400' : 'text-slate-550'} block tracking-wide uppercase`}>Rendimento Semanal</span>
                      <span className={`font-extrabold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>$ {sponsorIncome.toLocaleString('pt-BR')}</span>
                    </div>
                    <div>
                      <span className={`text-[8px] ${isDark ? 'text-slate-400' : 'text-slate-550'} block tracking-wide uppercase`}>Bônus de Assinatura</span>
                      <span className={`font-extrabold ${isDark ? 'text-emerald-400/90' : 'text-emerald-700'}`}>$ {sponsorBonus.toLocaleString('pt-BR')}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className={`p-5 rounded-xl border ${s.bgCard} shadow-sm space-y-3`}>
              <div className="flex items-center gap-2 border-b border-slate-200/10 dark:border-[#1e2d44]/30 pb-2.5">
                <Trophy className={`w-4 h-4 ${isDark ? 'text-yellow-400' : 'text-[#0B1B3D]'}`} />
                <h4 className={`text-xs font-black uppercase tracking-wider ${isDark ? 'text-white' : 'text-[#0B1B3D]'}`}>HISTÓRICO PROFISSIONAL</h4>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {teamTitles.map((title, idx) => (
                  <div 
                    key={idx} 
                    className={`p-3 rounded-lg flex items-center gap-2.5 font-bold text-xs uppercase ${s.bgInnerCard} border-l-2 border-yellow-405/80 ${isDark ? 'text-slate-205' : 'text-slate-800'}`}
                  >
                    <span className={`tracking-wide text-[10.5px] ${isDark ? 'text-slate-300' : 'text-slate-705'}`}>
                      {title}
                    </span>
                  </div>
                ))}
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className={`p-5 rounded-xl border ${s.bgCard} shadow-lg space-y-5 flex flex-col`}>
              <div className="flex items-center gap-2 border-b border-slate-200/10 dark:border-[#1e2d44]/30 pb-3">
                <Users className={`w-4.5 h-4.5 ${isDark ? 'text-sky-450' : 'text-[#0B1B3D]'}`} />
                <h4 className={`text-xs font-black uppercase tracking-wider ${isDark ? 'text-white' : 'text-[#0B1B3D]'}`}>
              ...
              {/* Titulares Section */}
              <div className="space-y-3">
                <span className={`block text-[9.5px] uppercase font-bold tracking-widest ${isDark ? 'text-sky-400' : 'text-[#0B1B3D]'}`}>
                  🛡️ TITULARES PRIMÁRIOS
                </span>
                <div className="space-y-2">
                  {activeTeam.roster.map((player) => {
                    const posColors: { [key: string]: string } = isDark ? {
                      ...
                      TOP: 'text-orange-900 bg-orange-100/60 border border-orange-300 font-bold',
                      JUG: 'text-emerald-900 bg-emerald-100/60 border border-emerald-300 font-bold',
                      MID: 'text-blue-900 bg-blue-100/60 border border-blue-300 font-bold',
                      ADC: 'text-purple-900 bg-purple-100/60 border border-purple-300 font-bold',
                      SUP: 'text-amber-950 bg-amber-100/60 border border-amber-300 font-bold'
                    };
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2.5 truncate">
                          ...
                          <div className="truncate">
                            <h5 className={`text-[11px] font-black hover:text-sky-400 transition-colors uppercase truncate ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>
                              {player.name}
                            </h5>
                            <p className={`text-[8px] mt-0.5 font-mono ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="text-right">
                            <span className={`text-[11px] font-mono font-extrabold block ${isDark ? 'text-[#00cbd6]' : 'text-sky-850'}`}>OVR {player.overallRating}</span>
                          </div>
                          {onSelectPlayer && (
                            <button
              ...
              <div className="space-y-3 pt-2">
                <span className={`block text-[9.5px] uppercase font-bold tracking-widest border-t border-slate-200/10 dark:border-[#1e2d44]/30 pt-3 ${isDark ? 'text-slate-400' : 'text-[#0B1B3D]'}`}>
                ...
                {activeTeam.substitutes && activeTeam.substitutes.length > 0 ? (
                  <div className="space-y-2">
                    {activeTeam.substitutes.map((player) => {
                      const posSubColors: { [key: string]: string } = isDark ? {
                        TOP: 'text-orange-400 bg-orange-500/10 border-orange-500/20',
                        JUG: 'text-green-400 bg-green-500/10 border-green-500/20',
                        MID: 'text-blue-400 bg-blue-500/10 border-blue-500/20',
                        ADC: 'text-purple-400 bg-purple-500/10 border-purple-500/20',
                        ...
                        TOP: 'text-orange-950 bg-orange-100/60 border border-orange-300 font-bold',
                        JUG: 'text-emerald-950 bg-emerald-100/60 border border-emerald-300 font-bold',
                        MID: 'text-blue-950 bg-blue-100/60 border border-blue-300 font-bold',
                        ADC: 'text-purple-950 bg-purple-100/60 border border-purple-300 font-bold',
                        SUP: 'text-amber-950 bg-amber-100/60 border border-amber-300 font-bold'
                      };
                      const subBadgeClass = posSubColors[player.position] || 'text-slate-405 bg-slate-500/10';
                      return (
                        <div 
                          key={player.id} 
                        ...
                        >
                          <div className="flex items-center gap-2.5 truncate">
                            <span className={`w-10 text-center text-[9px] font-black uppercase tracking-wider py-1 px-1.5 rounded-md border ${subBadgeClass}`}>
                              {player.position}
                            </span>
                            <div className="truncate">
                              <h5 className={`text-[11px] font-extrabold uppercase truncate ${isDark ? 'text-slate-200' : 'text-[#1A1A1A]'}`}>
                                {player.name}
                              </h5>
                              <p className={`text-[8px] mt-0.5 font-mono ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                                Idade: {player.age} anos • Potencial: {player.potential}
                          ...
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-mono font-bold ${isDark ? 'text-slate-300' : 'text-slate-850'}`}>OVR {player.overallRating}</span>
                            {onSelectPlayer && (
                              <button
                                onClick={() => onSelectPlayer(player.id)}
                                className={`text-[8.5px] font-bold uppercase tracking-wider duration-150 px-2 py-1 rounded cursor-pointer ${
                                  isDark ? 'text-slate-400 bg-slate-500/10 hover:bg-slate-500/20' : 'text-slate-800 bg-slate-200 hover:bg-slate-300'
                                }`}
                              >
                                Ver Atleta
                              </button>
                  ...
                  <p className={`text-[10px] italic ${isDark ? 'text-slate-500' : 'text-slate-600'}`}>Nenhum jogador reserva contratado no split principal.</p>
                )}
              </div>
...
export function EscritorioTab({ gameState, onUpdateGameState, triggerNotification, theme }: EscritorioTabProps) {
  ...
  const [internalAuditReport, setInternalAuditReport] = useState<any>(null);
  ...
  const formatRegulatoryTimer = (secs: number) => {
    const simulatedSeconds = secs * (1296000 / 300); // 4320
    const days = Math.floor(simulatedSeconds / (24 * 3600));
    const hours = Math.floor((simulatedSeconds % (24 * 3600)) / 3600);
    const minutes = Math.floor((simulatedSeconds % 3600) / 60);
  ...
  // Forced immediate compliance audit run
  const anticipateComplianceAudit = () => {
    ...
    const nextState = { ...gameState };
    const teamInState = nextState.teams.find(t => t.id === gameState.playerTeamId)!;

    if (isBudgetViolation || isSalaryGapViolation || hasActiveLoansViolation) {
      const issues: string[] = [];
      if (isBudgetViolation) issues.push(`Saldo de caixa negativo ($ ${playerTeam.budget.toLocaleString('pt-BR')})`);
      if (isSalaryGapViolation) issues.push(`Folha salarial ($ ${Math.round(basePlayerPayrollWeekly).toLocaleString('pt-BR')}) acima do teto de $ 120.005`);
      if (hasActiveLoansViolation) issues.push(`${playerTeam.loans.length} empréstimos financeiros ativos`);

      let fine = 0;
      if (isSalaryGapViolation) {
        const exceeded = basePlayerPayrollWeekly - salaryCapThreshold;
        fine = Math.round(exceeded * 1.50);
        teamInState.budget = Math.max(0, teamInState.budget - fine);
      }
      
      if (teamInState.ffpNonComplianceWeeks === undefined) {
        teamInState.ffpNonComplianceWeeks = 0;
      }
      teamInState.ffpNonComplianceWeeks++;

      let pointsDeducted = false;
      if (teamInState.ffpNonComplianceWeeks >= 24) {
        teamInState.points = Math.max(0, (teamInState.points || 0) - 3);
        teamInState.ffpNonComplianceWeeks = 0;
        pointsDeducted = true;
      }

      nextState.socialFeed.unshift({
        id: 'manual_compliance_audit_' + Math.random().toString(36).substr(2, 9),
        ...
        content: `🚨 [AUDITORIA ANTECIPADA] O clube ${playerTeam.name} solicitou auditoria voluntária. Status: REPROVADO! Detetado: ${issues.join(', ')}.${fine > 0 ? ` Cobrada Taxa de Luxo de $ ${fine.toLocaleString('pt-BR')}.` : ''}${pointsDeducted ? ' Perda de 3 pontos na tabela decretada!' : ''}`,
        likes: 5400,
        retweets: 1100,
        timeAgo: 'Agora',
        sentiment: 'negative',
      ...
      triggerNotification("🚨 Auditoria Concluída: Falha!", "Inconformidades estruturais e financeiras registradas pela liga.");
      alert(`🚨 RESULTADO DA AUDITORIA ANTECIPADA (RIOT COMPLIANCE)\n\nO clube foi auditado eletronicamente e encontra-se NÃO CONFORME devido a:\n${issues.map(t => `• ${t}`).join('\n')}\n\n${fine > 0 ? `💵 Multa de Taxa de Luxo Cobrada: $ ${fine.toLocaleString('pt-BR')}\n` : ''}${pointsDeducted ? `🚨 PENALIDADE ESPORTIVA: -3 pontos deduzidos do Split corrente!\n` : ''}Semanas acumuladas de não-conformidade (FFP): ${teamInState.ffpNonComplianceWeeks}/24.`);
    } else {
      if (teamInState.ffpNonComplianceWeeks !== undefined) {
        teamInState.ffpNonComplianceWeeks = 0;
      }
      ...
      nextState.socialFeed.unshift({
        ...
        handle: '@AuditoriaRivals',
        avatarUrl: 'https://images.unsplash.com/photo-1450133064473-71024230f91b?auto=format&fit=crop&q=80&w=100',
        content: `✅ [AUDITORIA ANTECIPADA] Auditoria voluntária no clube ${playerTeam.name}. Status: 100% REGULARIZADO! Sem pendências de fluxo ou teto.`,
        likes: 3100,
        retweets: 480,
        timeAgo: 'Agora',
        sentiment: 'positive',
      ...
      triggerNotification("✅ Auditoria Concluída: Sucesso!", "Clube aprovado em todos os critérios de conformidade da liga.");
      alert("✅ AUDITORIA CONCLUÍDA!\n\nStatus: TOTALMENTE EM COMPLIANCE\n\n• Caixa positivo: OK\n• Folha salarial abaixo de $120.000: OK\n• Sem empréstimo bancário em atraso ou aberto: OK\n\nSeu clube teve o histórico de semanas sob advertência zerado e a contagem reiniciada!");
    }
  };

  const runInternalAudit = () => {
    const activeLoans = playerTeam.loans || [];
    const loanIssues = activeLoans.map(l => {
      const weeklyReturn = Math.round(l.totalToPay / l.remainingWeeks);
      return {
        type: l.type,
        remaining: l.remainingWeeks,
        totalPayable: l.totalToPay,
        weeklyPayment: weeklyReturn
      };
    });

    const maxSalary = 120000;
    const weeklyPayroll = basePlayerPayrollWeekly;
    const payrollBreached = weeklyPayroll > maxSalary;
    const payrollDifference = maxSalary - weeklyPayroll;

    const importedPlayers = [...playerTeam.roster, ...playerTeam.substitutes].filter(p => p.isImported);
    const consularIssues = importedPlayers.map(p => {
      const hasAwaiting = playerTeam.vistasAwaiting?.find(v => v.playerId === p.id);
      return {
        name: p.name,
        visaApproved: p.visaApproved,
        hasAwaiting,
        weeksRemaining: hasAwaiting ? hasAwaiting.weeksRemaining : null,
        needsDocumentation: hasAwaiting ? hasAwaiting.hasDocumentationRequest : false
      };
    }).filter(i => !i.visaApproved);

    const expiringContracts = [...playerTeam.roster, ...playerTeam.substitutes].map(p => ({
      name: p.name,
    ...
    triggerNotification("📋 Diagnóstico Concluído!", "Parecer do nosso corpo jurídico e contabilidade gerado.");
  };
          ...
          <div className="space-y-3.5 max-h-[380px] overflow-y-auto pr-1">
            ...
            <div className={`p-3.5 rounded-xl border text-[10.5px] flex justify-between items-center ${isDark ? 'bg-slate-950/45 border-amber-500/20' : 'bg-amber-50/45 border-amber-200'}`}>
              ...
              <button 
                ...
                className={`px-1.5 py-0.5 rounded-md uppercase font-extrabold tracking-wider shadow text-[8.2px] cursor-pointer transition ${
                  playerTeam.investments.advancedSponsorWeeks > 0 
                  ...
                  : 'bg-gradient-to-r from-amber-500 to-yellow-600 hover:scale-[1.02] duration-150 text-white'
                }`}
              >
                {playerTeam.investments.advancedSponsorWeeks > 0 ? 'Bloqueado' : 'Antecipar'}
              </button>
            </div>
      ...
      <div className={`p-5 rounded-2xl border shadow-lg space-y-5 ${isDark ? 'border-[#1e2d44] bg-[#0a1424]' : 'border-slate-200 bg-white'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div key={player.id} className={`p-3 rounded-xl border flex justify-between items-center text-[10.5px] ${warningBorder}`}>
                    <div className="space-y-1 truncate flex-1">
                      ...
                      <div className={`flex flex-wrap items-center gap-3.5 text-[9.5px] ${isDark ? 'text-slate-400' : 'text-slate-550'}`}>
                        <span>Luvas de Contratação: <strong className="text-emerald-600 font-bold">{formatMoney(player.signOnFee || Math.round(player.marketValue * 0.25))}</strong></span>
                        <span>Salário Semanal: <strong className={isDark ? 'text-slate-200' : 'text-slate-800'}>{formatMoney(Math.round(player.salary / 4))}</strong></span>
                        <span>Multa Rescisória Internacional: <strong className="text-sky-600 font-bold">{formatMoney(Math.round(player.marketValue * 1.8))}</strong></span>
                      </div>
          ...
          <div id="visa-processing-center" className="lg:col-span-4 space-y-4">
            ...
            <div id="visa-notification-settings" className={`p-4 rounded-xl border text-[10.5px] space-y-3 ${
              isDark 
                ? 'bg-slate-950/45 border-indigo-550/15 text-slate-300' 
                : 'bg-[#f0f4f9] border-[#d2dfef] text-slate-900'
            }`}>
              <span className={`block text-[9.5px] font-black uppercase border-b pb-1.5 flex items-center gap-1.5 ${
                isDark ? 'text-indigo-400 border-slate-900' : 'text-[#312e81] border-[#d2dfef] font-black'
              }`}>
                <Clock className="w-3.5 h-3.5" />
                Configurar Alertas Semanais de Validade
              </span>
              <p className={`text-[9.2px] leading-normal ${isDark ? 'text-slate-400' : 'text-slate-700 font-medium'}`}>Configure para despachar lembretes de renovação de vistos para todos os estrangeiros 1 semana antes da perda do licenciamento consular:</p>
              
              <div className="space-y-2 pt-1 font-sans">
                {/* Control 1 */}
                <div className={`flex justify-between items-center p-1.5 rounded border ${
                  isDark ? 'bg-slate-900/40 border-transparent' : 'bg-white border-[#d2dfef]'
                }`}>
                  <span className={`text-[9.5px] font-bold ${isDark ? 'text-slate-300' : 'text-slate-900'}`}>Notificações Push Governamentais</span>
                  <button 
                ...
                <div className={`flex justify-between items-center p-1.5 rounded border ${
                  ...
                  <span className={`text-[9.5px] font-bold ${isDark ? 'text-slate-300' : 'text-slate-900'}`}>Filtrar Apenas Atletas Importados</span>
                  <button 
                ...
                <div className={`flex justify-between items-center p-1.5 rounded border ${
                  ...
                  <span className={`text-[9.5px] font-bold ${isDark ? 'text-slate-300' : 'text-slate-900'}`}>Notificação Crítica (Limiar 1 Semana)</span>
                  <span className={`px-2 py-0.2 rounded text-[9px] font-mono font-bold border ${isDark ? 'bg-slate-800 text-sky-450 border-slate-700' : 'bg-sky-50 text-sky-700 border-sky-200'}`}>Ativo</span>
                </div>
              </div>
            </div>
        ...
        <div className={`compliance-audit-section relative p-5 rounded-2xl border text-[10.5px] space-y-3.5 overflow-hidden transition duration-300 shadow-md ${
          isDark 
            ? 'bg-[#180a0f] border-red-500/25 border-l-[6px] border-l-red-650' 
            : 'bg-white border-slate-200 border-l-[6px] border-l-red-600'
        }`}>
          ...
          <div className={`flex justify-between items-center p-2 rounded-lg border ${
            isDark ? 'bg-red-950/40 border-red-500/15' : 'bg-red-50/80 border-red-200'
          }`}>
            <span className={`font-extrabold flex items-center gap-1.5 uppercase text-[9.5px] ${isDark ? 'text-red-400' : 'text-red-900 font-extrabold'}`}>
              ⏰ Próxima Janela de Auditoria (Riot Compliance):
            </span>
            <span className={`font-mono font-extrabold tracking-wider animate-pulse px-2 py-0.5 rounded text-[11px] border ${
              isDark 
                ? 'text-red-400 bg-red-900/25 border-red-500/20' 
                : 'text-red-950 bg-[#fee2e2] border-[#fca5a5]'
            }`}>
              {formatRegulatoryTimer(tickingTimer)}
            </span>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1.5 leading-relaxed flex-1">
              <h5 className={`font-black uppercase text-[11px] ${isDark ? 'text-white' : 'text-red-950'}`}>Checklist de Conformidades, Aliciamento e Auditorias Regulatórias</h5>
              <p className={isDark ? 'text-slate-300 font-medium' : 'text-slate-800 font-medium'}>
                Passar do teto salarial semanal da liga ($ 120.000) gera uma multa de Taxa de Fair-Play Financeiro de 150% do valor excedido e o não cumprimento do Fair-Play Financeiro reduz pontuação de 3 Pontos na tabela. Além disso, tentativas diretas de contatar e fazer propostas a atletas vinculados a clubes rivais sem rescisão acarretam a processos por aliciamento (Poaching).
              </p>
              <div className="pt-2 flex flex-wrap gap-2.5">
                <button
                  type="button"
                  onClick={anticipateComplianceAudit}
                  className={`px-3 py-1.5 uppercase font-extrabold text-[9px] rounded transition cursor-pointer border tracking-wider duration-150 active:scale-95 ${
                    isDark
                      ? 'bg-red-650/20 hover:bg-red-505 border-red-500/30 text-red-100 font-black'
                      : 'bg-red-600 hover:bg-red-700 text-white border-red-700 shadow-sm active:scale-95 duration-100'
                  }`}
                >
                  ⚖️ [ ANTECIPAR AUDITORIA ]
                </button>
                <button
                  type="button"
                  onClick={runInternalAudit}
                  className={`px-3 py-1.5 uppercase font-extrabold text-[9px] rounded transition cursor-pointer border tracking-wider duration-150 active:scale-95 ${
                    isDark
                      ? 'bg-sky-650/20 hover:bg-sky-505 border-sky-500/30 text-sky-100 font-black'
                      : 'bg-sky-600 hover:bg-sky-700 text-white border-sky-700 shadow-sm active:scale-95 duration-100'
                  }`}
                >
                  📋 [ AUDITORIA INTERNA ]
                </button>
          ...
          {/* Internal Audit Report Renderer */}
          {internalAuditReport && (
            <div className={`mt-3.5 p-4 rounded-xl border text-[10.5px] space-y-3 ${
              isDark 
                ? 'bg-slate-950/75 border-sky-500/20 text-slate-300' 
                : 'bg-sky-50/50 border-sky-100 text-slate-800'
            }`}>
              <div className="flex justify-between items-center border-b pb-2 border-sky-500/10">
                <span className="font-extrabold text-sky-400 uppercase flex items-center gap-1">
                  💼 PARECER JURÍDICO INTERNO (STAF ADVOGADOS)
                </span>
                <span className="font-mono text-[9px] text-slate-400">Diag: {internalAuditReport.timestamp}</span>
              </div>
              
              <div className="space-y-3.5">
                {/* Payroll Audit */}
                <div>
                  <span className="font-extrabold uppercase text-[10px] block mb-1">📊 Checagem de Teto de Folha (Limiar de $120.000):</span>
                  <div className="flex items-center gap-2 font-mono">
                    <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${internalAuditReport.payroll.breached ? 'bg-red-500/15 text-red-400' : 'bg-emerald-500/15 text-emerald-400'}`}>
                      {internalAuditReport.payroll.breached ? '🚨 ESTOURO' : '✔ COMPLIANT'}
                    </span>
                    <span>Atual: $ {Math.round(internalAuditReport.payroll.weekly).toLocaleString('pt-BR')}/semana</span>
                    <span className="text-slate-450">
                      ({internalAuditReport.payroll.breached 
                        ? `Excesso de $ ${Math.round(-internalAuditReport.payroll.difference).toLocaleString('pt-BR')}` 
                        : `$ ${Math.round(internalAuditReport.payroll.difference).toLocaleString('pt-BR')} sob o limite`})
                    </span>
                  </div>
                </div>

                {/* Bank Loans / Debts Audit */}
                <div>
                  <span className="font-extrabold uppercase text-[10px] block mb-1">🏦 Detecção de Empréstimos Bancários Ativos:</span>
                  {internalAuditReport.loans.length === 0 ? (
                    <p className="text-emerald-500 font-medium">✔ Nenhum empréstimo ativo em vigor (Passivo financeiro limpo!).</p>
                  ) : (
                    <div className="space-y-1.5">
                      {internalAuditReport.loans.map((l: any, idx: number) => (
                        <div key={idx} className="font-mono flex items-center justify-between pl-2 border-l-2 border-amber-500 bg-amber-500/5 py-1 pr-1 rounded">
                          <span>Empréstimo {l.type}</span>
                          <span>Restam: {l.remaining} semanas</span>
                          <span className="text-amber-400 font-bold">Dívida: $ {l.totalPayable.toLocaleString('pt-BR')} ($ {l.weeklyPayment.toLocaleString('pt-BR')}/sem)</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {/* Consular Visas Audit */}
                <div>
                  <span className="font-extrabold uppercase text-[10px] block mb-1">✈ Status e Vistos de Estrangeiros (Imports):</span>
                  {internalAuditReport.consular.length === 0 ? (
                    <p className="text-emerald-500 font-medium font-mono">✔ Todos os estrangeiros possuem Vistos P-1 / EB-1 regulares do Consulado.</p>
                  ) : (
                    <div className="space-y-1.5">
                      {internalAuditReport.consular.map((i: any, idx: number) => (
                        <div key={idx} className="font-mono flex items-center justify-between pl-2 border-l-2 border-red-500 bg-red-500/5 py-1 pr-1 rounded">
                          <span className="font-bold text-white">{i.name}</span>
                          <span className="text-red-400">Visto não aprovado (Irregular)</span>
                          {i.hasAwaiting ? (
                            <span className="text-slate-400">Tratando ({i.weeksRemaining} sem restante) {i.needsDocumentation && '⚠️ DOCUMENTAÇÃO INCOMPLETA'}</span>
                          ) : (
                            <span className="text-red-400 font-extrabold">🚨 NENHUM PROCESSO INICIADO</span>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                <div>
                  <span className="font-extrabold uppercase text-[10px] block mb-1">📜 Contratos em Fase de Pré-Contrato (Exempt Poaching):</span>
                  {internalAuditReport.contracts.length === 0 ? (
                    <p className="text-slate-400 font-mono">Nenhum contrato de jogador ativo expirando em menos de 6 meses no clube.</p>
                  ) : (
                    <div className="flex flex-wrap gap-1.5">
                      {internalAuditReport.contracts.map((c: any, idx: number) => (
                        <span key={idx} className="px-1.5 py-0.5 rounded bg-slate-900 border border-slate-700/50 text-[9px] font-mono text-slate-300">
                          {c.name} ({c.months} meses restantes)
                        </span>
                      ))}
                    </div>
                  )}
                ...
                <div className={`pt-2 border-t font-medium text-[10px] leading-relaxed ${
                  internalAuditReport.overallRecommendation.includes('IMPECÁVEL') ? 'text-emerald-400' : 'text-amber-400'
                }`}>
                  💡 {internalAuditReport.overallRecommendation}
                </div>
              </div>
            </div>
          )}
...
export function MetaTab({ gameState, theme }: MetaTabProps) {
  ...
  const [selectedRole, setSelectedRole] = useState<'TODAS AS ROTAS' | 'TOPO' | 'CAÇADOR' | 'MEIO' | 'ATIRADOR' | 'SUPORTE'>('TODAS AS ROTAS');
  ...
  const matchFilter = (roles: string[]) => {
    if (selectedRole === 'TODAS AS ROTAS') return true;
    const uppercaseRoles = roles.map(r => r.toUpperCase());
    if (selectedRole === 'TOPO') {
      return uppercaseRoles.includes('TOP') || uppercaseRoles.includes('BARON') || uppercaseRoles.includes('TOPO');
    }
    if (selectedRole === 'CAÇADOR') {
      return uppercaseRoles.includes('JUNGLE') || uppercaseRoles.includes('CAÇADOR') || uppercaseRoles.includes('CACADOR');
    }
    if (selectedRole === 'MEIO') {
      return uppercaseRoles.includes('MID') || uppercaseRoles.includes('MEIO');
    }
    if (selectedRole === 'ATIRADOR') {
      return uppercaseRoles.includes('ADC') || uppercaseRoles.includes('DUO') || uppercaseRoles.includes('ATIRADOR');
    }
    if (selectedRole === 'SUPORTE') {
      return uppercaseRoles.includes('SUPPORT') || uppercaseRoles.includes('SUPORTE');
    }
    return false;
  };

  const tiersInfo = [
    { name: 'S+', color: '#ff4a5a', label: 'Atualmente os melhores campeões do meta!' },
    { name: 'S', color: '#ff7a68', label: 'Picks de alto nível, altamente competitivos e dominantes.' },
    { name: 'A', color: '#ffb057', label: 'Escolhas fortes com grande utilidade tática e desempenho consistente.' },
    { name: 'B', color: '#ffd154', label: 'Picks equilibrados, ótimos para estratégias secundárias ou contra-escolhas.' },
    { name: 'C', color: '#7ee39d', label: 'Picks de nicho ou muito especializados, requerem boa composição de equipe.' },
    { name: 'D', color: '#94a3b8', label: 'Atualmente fora do meta, com dificuldades para impactar as partidas regularmente.' },
  ];

  const roleFilters: ('TODAS AS ROTAS' | 'TOPO' | 'CAÇADOR' | 'MEIO' | 'ATIRADOR' | 'SUPORTE')[] = [
    'TODAS AS ROTAS', 'TOPO', 'CAÇADOR', 'MEIO', 'ATIRADOR', 'SUPORTE'
  ];

  const getRoleLabelPt = (role: string) => {
    return role;
  };
        ...
        {roleFilters.map(role => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2.5 text-[11px] uppercase tracking-wider rounded-xl font-mono transition-all duration-200 cursor-pointer ${
                        ...
                        return (
                          <div key={champ.id} className="flex flex-col items-center justify-center w-24 sm:w-28 text-center select-none group relative">
                            ...
                            <div className="relative w-16 h-16 sm:w-18 sm:h-18">
                              ...
                              {/* Borda do Retrato */}
                              <div className="w-full h-full rounded-full border-2 border-slate-300 dark:border-[#38bdf8]/15 overflow-hidden relative shadow-md bg-transparent flex items-center justify-center">
                                <img 
                                  src={champion?.imageUrl || (champion?.image_blob ? getImageUrl(champion.image_blob) : getChampAvatar(champion.id, champion.imageSeed))} 
                                  onError={(e) => { (e.target as HTMLImageElement).src = 'assets/ui/fallback-champion.png'; }} 
                                  className="bg-transparent object-contain w-full h-full object-contain group-hover:scale-110 transition-transform duration-200 bg-transparent"
                                  referrerPolicy="no-referrer"
                                />
                              </div>
...
// ==========================================
// ==========================================
// 10. CARREIRA TAB (Manager Premium Hub)
// ==========================================

export function ensureManagers(gameState: GameState): GameState {
  if (!gameState || !gameState.teams) {
    return gameState;
  }

  // Identificar se todas as equipes já possuem manager cadastrado com o novo formato de ID
  const existingManagers = gameState.managers || [];
  const nextManagers: Manager[] = [];

  // Demissão automática de rival da IA caso o player passe a controlar esse time (estilo Brasfoot!)
  if (gameState.playerTeamId) {
    existingManagers.forEach(m => {
      if (m.teamId === gameState.playerTeamId && m.id !== 'player-manager') {
        m.teamId = null; // Vira Free Agent livre no mercado
        (m as any).status = 'Free Agent';
      }
    });
  }

  // 1. Identificar o manager do jogador e salvaguardar absolutamente
  const playerManager = existingManagers.find(m => m.id === 'player-manager') || existingManagers.find(m => m.teamId === gameState.playerTeamId);
  const playerTeam = gameState.playerTeamId ? gameState.teams.find(t => t.id === gameState.playerTeamId || t.isPlayerControlled) : null;

  if (playerManager) {
    // Manter o manager do jogador totalmente intocado e com ID player-manager
    const updatedPlayerManager: Manager = {
      ...playerManager,
      id: 'player-manager',
      teamId: playerTeam ? playerTeam.id : null,
      stats: playerTeam ? {
        ...playerManager.stats,
        wins: playerTeam.wins || playerManager.stats?.wins || 0,
        losses: playerTeam.losses || playerManager.stats?.losses || 0
      } : (playerManager.stats || { wins: 0, losses: 0, titles: 0, financialEfficiency: 80, youthPromotions: 0 })
    };
    nextManagers.push(updatedPlayerManager);
  } else {
    // Criar o manager inicial do jogador de forma segura caso não exista
    nextManagers.push({
      id: 'player-manager',
      name: gameState.managerName || 'Você',
      age: 32 + (gameState.season >= 2025 ? gameState.season - 2025 : (gameState.season || 1) - 1),
      nationality: 'Brasil',
      photoUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=150',
      teamId: playerTeam ? playerTeam.id : null,
      reputationTier: playerTeam ? (playerTeam.boardTrust >= 85 ? 'S' : playerTeam.boardTrust >= 70 ? 'A' : playerTeam.boardTrust >= 50 ? 'B' : 'C') : 'B',
      popularity: playerTeam ? (playerTeam.popularity || 80) : 60,
      style: 'Equilibrado',
      emotionalProfile: 'Analítico',
      stats: {
        wins: playerTeam ? (playerTeam.wins || 0) : 0,
        losses: playerTeam ? (playerTeam.losses || 0) : 0,
        titles: 1,
        financialEfficiency: 85,
        youthPromotions: playerTeam ? ((playerTeam.academy || []).length || 2) : 0
      },
      titlesGallery: [
        { date: 'Split Abertura', league: playerTeam ? (playerTeam.region || 'CBLOL') : 'CBLOL' }
      ],
      previousTeams: playerTeam ? [
        { teamName: 'G2 Esports', years: '2024' },
        { teamName: 'FURIA', years: '2023' }
      ] : [],
      recentResults: playerTeam ? (playerTeam.streak || 'WLWW').split('').filter(c => c === 'W' || c === 'L').slice(0, 5).map(c => c === 'W' ? 'V' : 'D') as ('V' | 'D')[] : [],
      topChampions: ['Azir', 'Orianna', 'Lee Sin']
    });
  }

  // 2. Iterar no ecossistema de equipes para criar e manter managers rivais com IDs baseados nas siglas (Ex: MGR_FUR, MGR_PNG)
  gameState.teams.forEach((t) => {
    const isPlayerTeam = gameState.playerTeamId && t.id === gameState.playerTeamId;
    if (isPlayerTeam) {
      return; // Pular para evitar alteração ou criação de rival na organização ativa do jogador
    }

    // Gerar ID limpo e padronizado baseado na sigla do time (MGR_ + SIGLA)
    const sigla = (t.acronym || t.id.slice(0, 3)).toUpperCase().replace(/[^A-Z0-9]/g, '');
    const cleanId = `MGR_${sigla}`;

    // Procurar se já existe um manager pra esse time rival cadastrado no sistema
    const existingRival = existingManagers.find(m => (m.teamId === t.id || m.id === cleanId) && m.id !== 'player-manager');

    if (existingRival) {
      // Atualiza mantendo propriedades originais e ID limpo
      nextManagers.push({
        ...existingRival,
        id: cleanId,
        teamId: t.id,
        stats: {
          ...existingRival.stats,
          wins: t.wins || existingRival.stats?.wins || 0,
          losses: t.losses || existingRival.stats?.losses || 0
        }
      });
    } else {
      // Criar novo manager genérico/fixo válido com o ID limpo baseado em sigla
      let name = '';
      let nats = 'Brasil';
      let tier: 'S' | 'A' | 'B' | 'C' = 'B';
      
      const region = t.region || 'CBLOL';
      if (region === 'LCK') {
        const kNames = ["Choi", "Kim", "Lee", "Park", "Jung", "Kang", "Shin", "Han", "Yoon"];
        const kFirsts = ["Kkoma", "Zefa", "DanDy", "Acorn", "NoFe", "Cain", "Fly", "Irean", "Score"];
        const charSum = t.id.split('').reduce((sum, c) => sum + c.charCodeAt(0), 0);
        const first = kFirsts[charSum % kFirsts.length];
        const last = kNames[charSum % kNames.length];
        name = `${first} ${last}`;
        nats = 'Coreia do Sul';
        tier = t.points > 12 ? 'S' : t.points > 6 ? 'A' : 'B';
      } else if (region === 'LPL') {
        const cNames = ["Wang", "Li", "Zhang", "Liu", "Chen", "Yang", "Huang", "Zhao", "Wu"];
        const cFirsts = ["Homme", "Tabe", "Kenzi", "Maokai", "Kash", "Chris", "Heart"];
        const charSum = t.id.split('').reduce((sum, c) => sum + c.charCodeAt(0), 0);
        const first = cFirsts[charSum % cFirsts.length];
        const last = cNames[charSum % cNames.length];
        name = `${first} ${last}`;
        nats = 'China';
        tier = t.points > 12 ? 'S' : t.points > 6 ? 'A' : 'B';
      } else if (region === 'LEC' || region === 'LCS') {
        const eFirsts = ["Dylan", "Mac", "Yamato", "Grabbz", "Peter", "Turtle", "Duffman", "Guilhoto", "Cain"];
        const eLasts = ["Falco", "Cannon", "Grzybek", "Coppen", "Mithy", "Nelson", "Delgado"];
        const charSum = t.id.split('').reduce((sum, c) => sum + c.charCodeAt(0), 0);
        const first = eFirsts[charSum % eFirsts.length];
        const last = eLasts[charSum % eLasts.length];
        name = `${first} ${last}`;
        nats = region === 'LEC' ? 'Europa' : 'Estados Unidos';
        tier = t.points > 12 ? 'S' : t.points > 6 ? 'A' : 'B';
      } else {
        const brFirsts = ["Maestro", "Djoko", "Halder", "Dionrray", "Sarkis", "Beone", "Glauber", "Cabu", "Riyev", "Alocs"];
        const brLasts = ["Silva", "Santos", "Oliveira", "Souza", "Rodrigues", "Ferreira", "Alves", "Barbosa"];
        const charSum = t.id.split('').reduce((sum, c) => sum + c.charCodeAt(0), 0);
        const first = brFirsts[charSum % brFirsts.length];
        const last = brLasts[charSum % brLasts.length];
        name = `Coach ${first} ${last}`;
        nats = 'Brasil';
        tier = t.wins > 10 ? 'S' : t.wins > 5 ? 'A' : 'B';
      }

      const styleOpts: ('Agressivo' | 'Equilibrado' | 'Lento')[] = ['Agressivo', 'Equilibrado', 'Lento'];
      const emoOpts: ('Estóico' | 'Explosivo' | 'Analítico' | 'Motivador')[] = ['Estóico', 'Explosivo', 'Analítico', 'Motivador'];
      const charCodeSum = t.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
      const style = styleOpts[charCodeSum % styleOpts.length];
      const emo = emoOpts[charCodeSum % emoOpts.length];

      nextManagers.push({
        id: cleanId,
        name,
        age: 27 + Math.floor((charCodeSum % 24)),
        nationality: nats,
        photoUrl: `https://images.unsplash.com/photo-${1500000000000 + (charCodeSum * 1234) % 950000}?auto=format&fit=crop&q=80&w=150`,
        teamId: t.id,
        reputationTier: tier,
        popularity: t.points ? Math.min(100, 40 + t.points * 3) : Math.floor(40 + Math.random() * 40),
        style,
        emotionalProfile: emo,
        stats: {
          wins: t.wins || 0,
          losses: t.losses || 0,
          titles: t.wins > 12 ? 1 : 0,
          financialEfficiency: Math.floor(62 + Math.random() * 28),
          youthPromotions: Math.floor(Math.random() * 3)
        },
        titlesGallery: t.wins > 12 ? [{ date: '25 Split Regular', league: t.region || 'CBLOL' }] : [],
        previousTeams: [],
        recentResults: (t.streak || 'WWL').split('').filter(c => c === 'W' || c === 'L').slice(0, 5).map(c => c === 'W' ? 'V' : 'D') as ('V' | 'D')[],
        topChampions: ['Renekton', 'Sejuani', 'Syndra']
      });
    }
  });

  // 3. Manter os managers desempregados cadastrados anteriomente
  existingManagers.forEach(m => {
    if (m.teamId === null && !nextManagers.some(nm => nm.id === m.id)) {
      nextManagers.push(m);
    }
  });
...
export function CarreiraTab({ gameState, theme, onUpdateGameState, triggerNotification }: CarreiraTabProps) {
  ...
  useEffect(() => {
    if (onUpdateGameState && (!gameState.managers || gameState.managers.length < gameState.teams.length)) {
      onUpdateGameState((prev) => {
        if (!prev) return prev;
        return ensureManagers(prev);
      });
    }
  ...
  const getReputationLabel = (mgr: any) => {
    ...
    const titles = mgr.stats?.titles || 0;
    if (tier === 'S' || titles >= 2) return 'Reputação Mundial';
    if (tier === 'A' || titles === 1) return 'Reputação Continental';
    if (tier === 'B') return 'Reputação Nacional';
    return 'Reputação Regional';
  };
  
  const [activeSubTab, setActiveSubTab] = useState<'profile' | 'directory' | 'market'>('profile');
  const [selectedManagerId, setSelectedManagerId] = useState<string>('player-manager');
  const [searchQuery, setSearchQuery] = useState('');
  const [regionFilter, setRegionFilter] = useState('ALL');
  const [simulationLogs, setSimulationLogs] = useState<string[]>([
    "Sessão de mercado iniciada para conselho de diretores.",
  ...
  const activeManager = managersList.find(m => m.id === selectedManagerId) || managersList.find(m => m.id === 'player-manager') || {
    id: 'player-manager',
    name: gameState.managerName,
    age: 32,
    nationality: 'Brasil',
    photoUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=150',
    teamId: gameState.playerTeamId,
    reputationTier: 'A' as const,
    popularity: 80,
    style: 'Equilibrado' as const,
    emotionalProfile: 'Analítico' as const,
    stats: { wins: 8, losses: 4, titles: 1, financialEfficiency: 82, youthPromotions: 2 },
    titlesGallery: [{ date: 'Split Abertura', league: 'CBLOL' }],
    previousTeams: [{ teamName: 'G2 Esports', years: '2024' }],
    recentResults: ['V', 'V', 'D', 'V', 'V'] as ('V' | 'D')[],
    topChampions: ['Azir', 'Orianna', 'Lee Sin']
  };

  const activeTeam = gameState.teams.find(t => t.id === activeManager.teamId);

  // Trigger background turn evaluation
  const runBoardSimulation = () => {
    if (!onUpdateGameState) return;

    onUpdateGameState((prev) => {
      if (!prev) return prev;
      const currentList = prev.managers || [];
      const logs: string[] = [];
      
      const nextList = currentList.map(mgr => {
        // Skip player
        if (mgr.id === 'player-manager') return mgr;

        // If high losses, there's a 35% chance of being fired!
        if (mgr.teamId && mgr.stats.losses >= mgr.stats.wins + 3 && Math.random() < 0.35) {
          const team = prev.teams.find(t => t.id === mgr.teamId);
          logs.push(`⚠️ DEMISSÃO: O Conselho do ${team ? team.name : 'Rival'} rescindiu contrato de ${mgr.name} devido a maus resultados corporativos!`);
          return {
            ...mgr,
            teamId: null,
            popularity: Math.max(10, mgr.popularity - 15),
            recentResults: ['D', 'D', 'D'] as ('V' | 'D')[]
          };
        }

        // If unemployed, 40% chance of being hired by an empty team!
        if (!mgr.teamId && Math.random() < 0.40) {
          // Find a team with no manager in managersList
          const emptyTeamObj = prev.teams.find(t => !currentList.some(m => m.teamId === t.id) && t.id !== prev.playerTeamId);
          if (emptyTeamObj) {
            logs.push(`🤝 CONTRATAÇÃO: O ${emptyTeamObj.name} anunciou ${mgr.name} como seu novo Manager Estratégico!`);
            return {
              ...mgr,
              teamId: emptyTeamObj.id,
              popularity: Math.min(100, mgr.popularity + 10),
              recentResults: ['V'] as ('V' | 'D')[]
            };
          }
        }
        return mgr;
      ...
      if (logs.length === 0) {
        logs.push("🔍 VISTORIA: Todas as comissões técnicas mantiveram estabilidade nesta semana.");
      }

      setSimulationLogs(prevLogs => [...logs, ...prevLogs].slice(0, 8));
      return {
        ...prev,
        managers: nextList
      };
  ...
  const totalGames = activeManager.stats.wins + activeManager.stats.losses;
  const winPercent = totalGames > 0 ? Math.round((activeManager.stats.wins / totalGames) * 100) : 60;
  const resilienceVal = Math.round(activeManager.stats.losses > 0 ? Math.min(100, (activeManager.stats.wins / activeManager.stats.losses) * 45) : 85);
  
  const radarData = [
    { subject: 'Vitória', A: winPercent },
    { subject: 'Resiliência', A: resilienceVal },
    { subject: 'Títulos', A: Math.min(100, activeManager.stats.titles * 30 + 40) },
    { subject: 'Financeiro', A: activeManager.stats.financialEfficiency },
    { subject: 'Revelação', A: Math.min(100, activeManager.stats.youthPromotions * 20 + 35) }
  ];

  // Visual classes matching theme
  const cardBgStyle = isDark ? 'bg-[#0a1424] border border-[#1e2d44] text-white' : 'bg-white border border-slate-200 shadow-sm text-slate-800';
  const headerBgStyle = isDark ? 'bg-[#080d19]' : 'bg-slate-100 text-slate-900 border border-slate-200';
  const textTitleStyle = isDark ? 'text-white' : 'text-[#000000]';
  const textMutedStyle = isDark ? 'text-slate-400' : 'text-slate-500';
  const activeTabStyle = 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30';
  const inactiveTabStyle = isDark ? 'hover:bg-slate-800 border border-transparent text-slate-400' : 'hover:bg-slate-200 border border-transparent text-slate-600';
  ...
  const filteredManagers = managersList.filter(m => {
    ...
    if (regionFilter === 'DESEMPREGADO') return m.teamId === null;
    
    const team = gameState.teams.find(t => t.id === m.teamId);
    return team && team.region === regionFilter;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className={`p-1.5 rounded-md border ${isDark ? 'bg-[#00E5FF]/10 border-[#00E5FF]/20 text-cyan-400' : 'bg-cyan-50 border-cyan-200 text-cyan-600'}`}>
              <Crown className="w-5 h-5 animate-pulse" />
            </span>
            <span className={`text-[10px] uppercase tracking-widest font-extrabold ${textMutedStyle}`}>ECOSSISTEMA DE MANAGER</span>
          </div>
          <h1 className={`${textTitleStyle} text-xl md:text-2xl font-black uppercase tracking-tight mt-1`}>
            Central & Legado de Carreira
          </h1>
          <p className={`text-xs mt-0.5 ${textMutedStyle}`}>
            Monitore seu legado, perfis de filosofia de draft e o ecossistema ativo de managers rivais das ligas globais.
          </p>
        </div>

        {/* Dynamic sub tab selector */}
        <div className={`p-1 rounded-xl flex gap-1 border ${isDark ? 'bg-slate-950/40 border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
          <button 
            onClick={() => { setActiveSubTab('profile'); setSelectedManagerId('player-manager'); }}
            className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold uppercase transition-all tracking-wider ${activeSubTab === 'profile' ? activeTabStyle : inactiveTabStyle}`}
          >
            Meu Perfil
          </button>
          <button 
            onClick={() => setActiveSubTab('directory')}
            className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold uppercase transition-all tracking-wider ${activeSubTab === 'directory' ? activeTabStyle : inactiveTabStyle}`}
          >
            Managers Rivais ({managersList.length - 1})
          </button>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        {/* LEFT COLUMN/SIDE PANEL (MANAGER LIST OR ACTION LOGS) */}
        <div className="lg:col-span-1 space-y-6">
          
          {activeSubTab === 'profile' ? (
            <div className={`p-5 rounded-2xl border ${cardBgStyle} space-y-4`}>
              <h3 className="text-xs font-black tracking-widest uppercase">CONQUISTAS RECENTES</h3>
              <div className="space-y-3 font-mono text-[11px]">
                <div className={`p-3 rounded-xl flex items-center justify-between ${isDark ? 'bg-slate-950/40' : 'bg-slate-50'}`}>
                  <span>STRETCH MASTER</span>
                  <span className="text-emerald-400 font-extrabold">COMPLETADO</span>
                </div>
                <div className={`p-3 rounded-xl flex items-center justify-between ${isDark ? 'bg-slate-950/40' : 'bg-slate-50'}`}>
                  <span>ESTABILIDADE FISCAL</span>
                  <span className="text-cyan-400 font-extrabold">COMPLETADO</span>
                </div>
                <div className={`p-3 rounded-xl flex items-center justify-between opacity-50 ${isDark ? 'bg-slate-950/40' : 'bg-slate-50'}`}>
                  <span>CAMPEÃO WORLDS</span>
                  <span className="text-slate-500 font-extrabold">BLOQUEADO</span>
                </div>
              </div>
            </div>
          ) : (
            <div className={`p-5 rounded-2xl border ${cardBgStyle} space-y-4`}>
              <div className="flex justify-between items-center pb-2 border-b border-slate-500/10">
                <h3 className="text-xs font-black tracking-widest uppercase">PESQUISA DE MANAGERS</h3>
                <span className="px-2 py-0.5 rounded text-[8px] font-black tracking-widest bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 uppercase">ATIVO</span>
              </div>

              {/* SEARCH */}
              <div className="space-y-1.5">
                <label className="text-[9.5px] font-black text-slate-400 tracking-wider uppercase block">NOME DO GERENTE</label>
                <input
                  type="text"
                  placeholder="Buscar manager..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className={`w-full p-2.5 rounded-lg text-xs font-mono border focus:outline-none focus:border-cyan-500 ${isDark ? 'bg-slate-950 border-slate-800 text-white' : 'bg-slate-50 border-slate-300 text-slate-900'}`}
                />
              </div>

              {/* REGION SEPARATOR */}
              <div className="space-y-1.5">
                <label className="text-[9.5px] font-black text-slate-400 tracking-wider uppercase block">COMPREENSÃO REGIONAL</label>
                <select
                  value={regionFilter}
                ...
                >
                  ...
                  <option value="LCS">LCS (AMÉRICA)</option>
                  <option value="DESEMPREGADO">LIVRES (SEM ORGANIZAÇÃO)</option>
                </select>
              </div>
              ...
              <div className="pt-3 border-t border-slate-500/10">
                <button
                  ...
                  className="w-full bg-cyan-600 hover:bg-cyan-500 text-white text-[10px] font-black py-3 px-4 uppercase tracking-wider rounded-xl transition-all cursor-pointer shadow-sm flex items-center justify-center gap-2"
                >
                  <Search className="w-3.5 h-3.5" />
                  LIMPAR FILTROS DE PESQUISA
                </button>
                <p className="text-[9.5px] leading-relaxed text-slate-500 mt-2 text-center">
                  Redefine todos os filtros de nome e região para listar todos os gerentes e managers rivais do ecossistema de dados.
                </p>
              </div>
            </div>
          )}

          {/* SIMULATION LIVE FEED NEWS */}
          <div className={`p-5 rounded-2xl border ${cardBgStyle} space-y-3`}>
            <div className="flex items-center gap-1.5">
              <Terminal className="w-4 h-4 text-cyan-400" />
              <h3 className="text-xs font-black tracking-widest uppercase">MUDANÇAS DE MERCADO / STAFF</h3>
            </div>
            <div className="space-y-2 h-[200px] overflow-y-auto pr-1 scrollbar-thin text-[10.5px]">
              {simulationLogs.map((log, idx) => (
                <div key={idx} className={`p-2.5 rounded-lg border leading-tight ${isDark ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                  {log}
                </div>
              ))}
            </div>
        ...
        {/* RIGHT COLUMN/MAIN BODY (DETAILED MANAGER PROFILE OR DIRECTORY TABLE) */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* RENDER ACTIVE DIRECTORY OF ALL RIVAL MANAGERS */}
          {activeSubTab === 'directory' ? (
            <div className={`p-6 rounded-2xl border ${cardBgStyle} space-y-4`}>
              <div className="flex justify-between items-center pb-2 border-b border-slate-500/10">
                <h3 className="text-xs font-black tracking-widest uppercase">DIRETÓRIO ATIVO DE MANAGERS RIVAIS</h3>
                <span className="text-[10px] text-slate-500 font-bold">{filteredManagers.length} cadastrados</span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[640px] overflow-y-auto pr-1 scrollbar-thin">
                {filteredManagers.map((mgr) => {
                  const team = gameState.teams.find(t => t.id === mgr.teamId);
                  const isPlayer = mgr.id === 'player-manager';
                  const total = mgr.stats.wins + mgr.stats.losses;
                  const winRate = total > 0 ? Math.round((mgr.stats.wins / total) * 100) : 50;

                  return (
                    <div 
                      key={mgr.id} 
                      onClick={() => { setSelectedManagerId(mgr.id); setActiveSubTab('profile'); }}
                      className={`p-4 rounded-xl border flex gap-3 cursor-pointer transition-all hover:scale-[1.01] ${selectedManagerId === mgr.id ? 'border-cyan-500 bg-cyan-500/5 shadow-md' : (isDark ? 'bg-slate-900 border-slate-800/60 hover:bg-slate-850' : 'bg-slate-50 border-slate-200 hover:bg-slate-100')}`}
                    >
                      <div className="w-12 h-12 rounded-xl border border-slate-500/20 overflow-hidden relative bg-slate-800 shrink-0 flex items-center justify-center">
                        <img 
                          referrerPolicy="no-referrer" 
                        ...
                        />
                        {isPlayer && (
                          <div className="absolute top-0 right-0 p-0.5 bg-yellow-500 text-black text-[7px] font-black uppercase rounded-bl">VOCÊ</div>
                        )}
                      </div>

                      <div className="flex-1 space-y-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <h4 className="font-bold text-[11.5px] truncate text-slate-200 leading-tight block" style={{ color: isDark ? '#ffffff' : '#0f172a' }}>
                            {mgr.name}
                          </h4>
                          <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border tracking-wider uppercase ${mgr.reputationTier === 'S' ? 'text-red-400 bg-red-400/5 border-red-400/20' : (mgr.reputationTier === 'A' ? 'text-cyan-400 bg-cyan-400/5 border-cyan-400/30' : 'text-slate-400 bg-slate-400/5')}`}>
                            TIER {mgr.reputationTier}
                          </span>
                        </div>

                        <div className="flex justify-between items-center text-[10px]">
                          <span className={textMutedStyle}>{mgr.nationality} • {mgr.age} anos</span>
                          <span className="font-mono font-bold text-cyan-400">{winRate}% WR</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-slate-500/10">
                          {team ? (
                            ...
                            <span className="text-amber-500 text-[9px] font-bold uppercase tracking-wider">LIVRE / SEM CLUBE</span>
                          )}
                          <span className={`${textMutedStyle} text-[9px] tracking-wider uppercase font-mono`}>{mgr.style}</span>
                        </div>
                      </div>
                    </div>
                  );
                ...
                {filteredManagers.length === 0 && (
                  <div className="col-span-2 py-16 text-center text-slate-550 border border-dashed border-slate-800 rounded-xl">
                    <Users className="w-10 h-10 mx-auto mb-2 opacity-40 text-cyan-400" />
                    <p className="font-bold text-xs uppercase tracking-widest">Nenhum Manager Encontrado</p>
                    <p className="text-[11px] mt-1 text-slate-500">Tente ajustar seus termos de busca ou filtros regionais.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            // FULL INTERACTIVE DETAILED PROFILE (Módulos A, B, C, D)
            <div className="space-y-6">
              
              {/* MÓDULO A: CABEÇALHO DE IDENTIDADE E DESTAQUE (HERO BLOCK) */}
              <div className={`p-6 rounded-2xl border relative overflow-hidden flex flex-col justify-between ${cardBgStyle}`}>
                {/* Visual red circular contour contour marker overlay back */}
                {isDark && (
                  <div className="absolute top-0 right-0 w-[240px] h-[240px] bg-cyan-500/5 rounded-full filter blur-2xl pointer-events-none" />
                )}

                {/* Sub row hierarchy alignment */}
                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                  
                  {/* Bio details elements */}
                  <div className="flex gap-4 items-center">
                    <input 
                      type="file" 
                      ref={fileInputRef} 
                      ...
                      onChange={(e) => {
                        ...
                        if (file && onUpdateGameState) {
                          const reader = new FileReader();
                          reader.onload = () => {
                            const arrayBuffer = reader.result as ArrayBuffer;
                            
                            // Convert standard base64 for localstorage/compat
                            const b64Reader = new FileReader();
                            b64Reader.onload = () => {
                              const base64Url = b64Reader.result as string;
                              localStorage.setItem('legendshub_manager_avatar', base64Url);
                              
                              onUpdateGameState((prev) => {
                                if (!prev) return prev;
                                const updatedManagers = (prev.managers || []).map(m => {
                                  if (m.id === 'player-manager') {
                                    return { ...m, photoUrl: base64Url, image_blob: arrayBuffer };
                                  }
                                  return m;
                                });

                                // Execute IndexedDB UPDATE on managers list
                                setGameItem('legendshub_custom_managers', updatedManagers).catch(err => {
                                  console.error('[IndexedDB] Error saving custom managers', err);
                                });

                                return {
                                  ...prev,
                                  managerPhoto: base64Url,
                    ...
                    <div 
                      onClick={() => {
                        if (activeManager.id === 'player-manager') {
                          fileInputRef.current?.click();
                        }
                      }}
                      className={`w-20 h-20 rounded-full border-2 border-cyan-400 overflow-hidden shrink-0 shadow-inner bg-slate-800 relative flex items-center justify-center ${activeManager.id === 'player-manager' ? 'cursor-pointer hover:border-cyan-300 hover:scale-[1.05] transition-all duration-150 group' : ''}`}
                      title={activeManager.id === 'player-manager' ? "Clique para alterar sua foto de perfil" : undefined}
                    >
                      <img 
                        referrerPolicy="no-referrer" 
                        src={activeManager.id === 'player-manager' ? (activeManager.image_blob ? getImageUrl(activeManager.image_blob) : activeManager.photoUrl || getGameAssetUrl('managers', activeManager.id, activeManager.photoUrl)) : getGameAssetUrl('managers', activeManager.id, activeManager.photoUrl)} 
                        alt={activeManager.name} 
                        className="w-full h-full object-cover bg-transparent" 
                        onError={(e) => { (e.target as HTMLImageElement).src = 'assets/ui/fallback-player.png'; }}
                      />
                      {activeManager.id === 'player-manager' && (
                        <div className="absolute inset-x-0 bottom-0 bg-black/60 text-[7px] py-1 font-mono font-black text-cyan-400 uppercase tracking-widest text-center transition-opacity flex items-center justify-center select-none">
                          ATUALIZAR
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h2 className={`font-black text-lg md:text-xl uppercase tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{activeManager.name}</h2>
                        {activeManager.id === 'player-manager' && (
                          <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-yellow-500/10 text-yellow-500 border border-yellow-500/20">VOCÊ</span>
                        )}
                      </div>
                      <p className={`text-[11.5px] font-semibold ${textMutedStyle}`}>
                        {activeManager.age} anos • {activeManager.nationality}
                      </p>
                      
                      <div className="flex items-center gap-1.5 pt-1.5">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black tracking-widest border border-cyan-400/20 bg-cyan-400/5 text-cyan-400`}>
                          TIER {activeManager.reputationTier}
                        </span>
                        <span className="text-[10px] text-slate-500 font-bold">• {getReputationLabel(activeManager)}</span>
                      </div>
                    </div>
                  </div>

                  {/* Destaque Corporativo: LOGO under circular marker (Demarcado por círculo vermelho com o rótulo "LOGO" do layout figma) */}
                  <div className="self-end md:self-start flex flex-col items-end gap-1 select-none pr-1">
                    <div className="relative group p-1.5 rounded-3xl border border-slate-200 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/45 shadow-inner">
                      {/* Logo container overlay */}
                      <span className="absolute -top-3 right-2 text-[7.5px] font-black font-mono tracking-widest bg-slate-800 dark:bg-slate-950 text-slate-200 dark:text-slate-300 border border-slate-700 dark:border-slate-800 px-1.5 py-0.5 uppercase rounded-sm z-10 font-sans">TIME ATIVO</span>
                      
                       {activeTeam ? (
                        <div 
                          className="w-20 h-20 rounded-2xl flex flex-col items-center justify-center font-black text-white shrink-0 border relative overflow-hidden shadow-lg transition-transform p-1.5"
                          style={{
                            background: activeTeam?.logoUrl || getGameAssetUrl('teams', activeTeam?.id) ? (isDark ? '#09101f' : '#f8fafc') : `linear-gradient(135deg, ${activeTeam?.primaryColor || '#1e293b'}, ${activeTeam?.secondaryColor || '#0f172a'})`,
                            borderColor: activeTeam?.primaryColor || '#1e293b'
                          }}
                        >
                          <img 
                            src={getGameAssetUrl('teams', activeTeam?.id, activeTeam?.logoUrl)} 
                            alt={activeTeam?.name}
                        ...
                        <div className="w-20 h-20 rounded-2xl flex flex-col items-center justify-center font-black text-slate-400 border border-dashed border-slate-700 bg-slate-900 shadow-lg">
                          <span className="text-xl uppercase tracking-widest">LIVRE</span>
                          <span className="text-[8px] uppercase tracking-wider text-slate-500 block mt-0.5">SEM CLUB</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-500/10 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <div className="flex justify-between items-baseline text-[10px]">
                      <span className="font-black text-slate-400 tracking-wider">AENGAGAMENTO / POPULARIDADE</span>
                      <span className="font-mono font-black text-[#00E5FF]">{activeManager.popularity}%</span>
                    </div>
                    <div className={`w-full h-2 rounded-full overflow-hidden ${isDark ? 'bg-slate-950' : 'bg-slate-100 border border-slate-200'}`}>
                      <div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-indigo-500 transition-all duration-500" style={{ width: `${activeManager.popularity}%` }} />
                    </div>
                  </div>

                  <div className="text-[10px] leading-relaxed text-slate-500 flex items-center">
                    {activeTeam ? (
                      <span>Atualmente vinculado com a organização <strong>{activeTeam.name}</strong>, sob fiscalização ativa do Split.</span>
                    ) : (
                      <span>Manager está atualmente livre no mercado global, passível de novas abordagens contratuais.</span>
                    )}
                  </div>
                </div>
              </div>

              {/* TWO SECTIONS GRID SIDES */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
                
                {/* MÓDULO B: FILOSOFIA DE TRABALHO E ATRIBUTOS */}
                <div className={`p-5 rounded-2xl border flex flex-col justify-between ${cardBgStyle}`}>
                  <div className="space-y-4">
                    <div className="flex items-center gap-1.5 pb-2 border-b border-slate-500/10">
                      <Briefcase className="w-4 h-4 text-cyan-400" />
                      <h3 className="text-xs font-black tracking-widest uppercase">FILOSOFIA & DIRETRIZ TÁTICA</h3>
                    </div>

                    <div className="space-y-3.5">
                      {/* Mentalidade of Draft */}
                      <div className="space-y-1">
                        <span className="text-[9.5px] font-black text-slate-500 uppercase tracking-wider block">Estilo de Draft Preferencal</span>
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded text-[10.5px] font-black tracking-wider uppercase border ${
                            activeManager.style === 'Agressivo' ? 'text-red-400 border-red-500/20 bg-red-500/5' :
                            activeManager.style === 'Lento' ? 'text-blue-400 border-blue-500/20 bg-blue-500/5' :
                            'text-cyan-400 border-cyan-450/20 bg-cyan-450/5'
                          }`}>
                            {activeManager.style}
                          </span>
                        </div>
                        <p className={`text-[10.5px] leading-relaxed mt-1.5 ${textMutedStyle}`}>
                          {activeManager.style === 'Agressivo' ? "Prioriza composições de skirmish focado em rotas combativas e early game." : 
                           activeManager.style === 'Lento' ? "Condução focada em lutas de teamfights e forte scaling de ouro em late game." : 
                           "Garante flexibilidade tática adaptando-se livremente de acordo com o adversário."}
                        </p>
                      </div>

                      {/* Emotional Profile */}
                      <div className="space-y-1 pt-2 border-t border-slate-550/10">
                        <span className="text-[9.5px] font-black text-slate-500 uppercase tracking-wider block">Perfil de Gestão Comportamental</span>
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-0.5 rounded text-[10.5px] font-black tracking-wider uppercase border border-slate-500/20 bg-slate-500/5">
                            {activeManager.emotionalProfile}
                          </span>
                        </div>
                        <p className={`text-[10.5px] leading-relaxed mt-1.5 ${textMutedStyle}`}>
                          {activeManager.emotionalProfile === 'Analítico' ? "Absoluta frieza diante de derrotas, ancorado em dados quantitativos puros." :
                           activeManager.emotionalProfile === 'Motivador' ? "Mantém união moral e eleva química com foco em saúde mental do elenco." :
                           activeManager.emotionalProfile === 'Estóico' ? "Serenidade tática para blindar os atletas da pressão extrema das redes sociais." :
                           "Exalta as cobranças metodológicas e exige alta performance sob risco de rebaixamentos."}
                        </p>
                      </div>
                    </div>
                  </div>

                  <div className={`mt-4 p-2.5 rounded-lg text-[9.5px] leading-relaxed select-none ${isDark ? 'bg-slate-950/40 text-slate-400 border border-slate-800' : 'bg-slate-100 border'}`}>
                    <strong>NOTA TÁTICA:</strong> O estilo tático dita a afinidade com composições do draft na simulação da liga.
                  </div>
                </div>

                {/* MÓDULO C: GRÁFICO POLIGONAL DE PERFORMANCE (RADAR CHART) */}
                <div className={`p-5 rounded-2xl border flex flex-col justify-between ${cardBgStyle}`}>
                  <div>
                    <div className="flex items-center gap-1.5 pb-2 border-b border-[#1e2d44]/50">
                      <BarChart3 className="w-4 h-4 text-cyan-400" />
                      <h3 className="text-xs font-black tracking-widest uppercase">POLÍGONO DE PERFORMANCE</h3>
                    </div>
                    <p className={`text-[10px] mt-1 ${textMutedStyle}`}>Atributos calibrados dinamicamente com base em resultados históricos.</p>
                  </div>

                  {/* RADAR GRÁFICO usando Recharts */}
                  <div className="my-2 relative flex items-center justify-center w-full min-h-[220px]">
                    <ResponsiveContainer width="100%" height={230}>
                      <RadarChart cx="50%" cy="50%" outerRadius="62%" data={radarData}>
                        <PolarGrid stroke={isDark ? "#1e2d44" : "#e2e8f0"} />
                        <PolarAngleAxis 
                          dataKey="subject" 
                          tick={{ fill: isDark ? "#94a3b8" : "#475569", fontSize: 9, fontWeight: 'bold' }} 
                        />
                        <PolarRadiusAxis 
                          angle={30} 
                          domain={[0, 100]} 
              ...
              {/* MÓDULO D: MEMÓRIA INSTITUCIONAL E ESTATÍSTICA (HISTÓRICO) */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-stretch">
                
                {/* 1. TÍTULOS */}
                <div className={`p-4 rounded-xl border flex flex-col justify-between text-xs md:col-span-1 ${cardBgStyle}`}>
                  <div className="space-y-2">
                    <div className="flex items-center gap-1 pb-1 border-b border-slate-500/10">
                      <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                      <span className="text-[10px] font-black uppercase tracking-wider">TÍTULOS COLECIONADOS</span>
                    </div>

                    <div className="space-y-2 mt-2">
                      {activeManager.titlesGallery && activeManager.titlesGallery.length > 0 ? (
                        activeManager.titlesGallery.map((tit, i) => (
                          <div key={i} className={`p-2 rounded border flex justify-between items-center ${isDark ? 'bg-slate-950/40 border-slate-900' : 'bg-slate-50 border-slate-205'}`}>
                            <span className="font-extrabold text-[10px] uppercase truncate">{tit.date}</span>
                            <span className="text-[10.5px] font-bold text-yellow-500">{tit.league}</span>
                          </div>
                        ))
                      ) : (
                        <div className="py-6 text-center text-slate-500 text-[10px]">
                          Sem Troféus Oficiais CBLOL
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="text-[9px] text-slate-500 font-mono text-center pt-2">
                    Total: {activeManager.stats.titles} Ouro(s)
                  </div>
                </div>
                ...
                <div className={`p-4 rounded-xl border flex flex-col justify-between text-xs md:col-span-1 ${cardBgStyle}`}>
                  <div className="space-y-2">
                    <div className="flex items-center gap-1 pb-1 border-b border-slate-500/10">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-wider">CLUBES ANTERIORES</span>
                    </div>

                    <div className="space-y-2 mt-2 max-h-[100px] overflow-y-auto pr-1 scrollbar-thin">
                      {activeManager.previousTeams && activeManager.previousTeams.length > 0 ? (
                        activeManager.previousTeams.map((pt, i) => {
                          const foundTeam = gameState.teams.find((t: any) => 
                            t.name.toLowerCase() === pt.teamName.toLowerCase() || 
                            t.acronym.toLowerCase() === pt.teamName.toLowerCase() ||
                            (pt.teamName.toLowerCase().includes('g2') && t.acronym === 'G2') ||
                            t.name.toLowerCase().includes(pt.teamName.toLowerCase()) ||
                            pt.teamName.toLowerCase().includes(t.name.toLowerCase())
                          );

                          return (
                            <div key={i} className="flex justify-between items-center text-[10px] pb-1 border-b border-dashed border-slate-500/10">
                              <div className="flex items-center gap-1.5 min-w-0">
                                {foundTeam && (foundTeam.logoUrl || getGameAssetUrl('teams', foundTeam.id)) ? (
                                  <img 
                                    src={getGameAssetUrl('teams', foundTeam?.id, foundTeam?.logoUrl)} 
                                  ...
                                  <div 
                                    className="w-4 h-4 rounded text-[7px] font-black font-mono flex items-center justify-center shrink-0 text-white" 
                                    style={{ 
                                      background: `linear-gradient(135deg, ${foundTeam.primaryColor || '#1e293b'}, ${foundTeam.secondaryColor || '#0f172a'})` 
                                    }}
                                  >
                                    {foundTeam.acronym}
                                  </div>
                                ...
                                <span className="font-semibold truncate">{pt.teamName}</span>
                              </div>
                              <span className="font-mono text-slate-500">{pt.years}</span>
                            </div>
                          );
                        })
                      ) : (
                        <div className="py-6 text-center text-slate-500 text-[10px]">
                          Carreira Iniciada Recente
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* 3. ÚLTIMOS 5 RESULTADOS */}
                <div className={`p-4 rounded-xl border flex flex-col justify-between text-xs md:col-span-1 ${cardBgStyle}`}>
                  <div className="space-y-2">
                    <div className="flex items-center gap-1 pb-1 border-b border-slate-500/10">
                      <Zap className="w-3.5 h-3.5 text-amber-400" />
                      <span className="text-[10px] font-black uppercase tracking-wider">ÚLTIMOS CONFRONTOS</span>
                    </div>

                    <div className="flex justify-center items-center gap-2 py-4 mt-2">
                      {activeManager.recentResults && activeManager.recentResults.length > 0 ? (
                        activeManager.recentResults.slice(0, 5).map((res, i) => (
                          <span 
                            key={i} 
                            className={`w-7 h-7 rounded-full flex items-center justify-center font-black text-xs border ${
                              res === 'V' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-450' : 'bg-red-500/10 border-red-500/30 text-red-400'
                            }`}
                          >
                            {res}
                          </span>
                        ...
                        <div className="text-slate-500 text-[10px]">Estreando na temporada regular</div>
                      )}
                    </div>
                  </div>

                  <div className="text-[9px] text-slate-550 font-mono text-center pt-1 leading-none">
                    Forma: {activeManager.recentResults?.join('-') || 'N/A'}
                  </div>
                </div>

                {/* 4. METAGAME CHAMPIONS ASSINATURA */}
                <div className={`p-4 rounded-xl border flex flex-col justify-between text-xs md:col-span-1 ${cardBgStyle}`}>
                  <div className="space-y-2">
                    <div className="flex items-center gap-1 pb-1 border-b border-slate-500/10">
                      <Gamepad2 className="w-3.5 h-3.5 text-indigo-400" />
                      <span className="text-[10px] font-black uppercase tracking-wider">DRAFTS PREFERIDOS</span>
                    </div>

                    <div className="space-y-2 mt-2">
                      {activeManager.topChampions && activeManager.topChampions.map((champ, id) => (
                        <div key={id} className={`p-1 px-2 rounded-lg text-[9.5px] flex justify-between items-center ${isDark ? 'bg-slate-950/20' : 'bg-slate-50'}`}>
                          <span className="font-semibold">{champ}</span>
                          <span className="text-[8px] px-1 bg-cyan-500/10 text-cyan-400 rounded">Hero</span>
                        </div>
                      ))}
                    </div>
                  </div>

                  <div className="text-[9.5px] text-slate-500 text-center leading-none mt-1">
                    Meta Combo
                  </div>
                </div>
...
export function SalvarJogoTab({ gameState, onManualSave, triggerNotification }: SalvarJogoTabProps) {
  ...
  const fetchSaveSlotsFromBrowser = async () => {
    const list = await Promise.all([1, 2, 3].map(async (i) => {
      const parsed = await getGameItem<GameState>(`legendshub_save_slot_${i}`);
      if (parsed) {
        try {
          const savedDate = await getGameItem<string>(`legendshub_save_slot_${i}_date`) || 'N/A';
  ...
  const handleDeleteSlotLocal = async (slotIdx: number) => {
    if (confirm(`Excluir permanentemente o Slot de Salvamento ${slotIdx}?`)) {
      await removeGameItem(`legendshub_save_slot_${slotIdx}`);
      await removeGameItem(`legendshub_save_slot_${slotIdx}_date`);
      fetchSaveSlotsFromBrowser();
...
const getStaffIcon = (cargo: string, departamento?: string) => {
  ...
  if (dept.includes('MARKETING') || c.includes('PR') || c.includes('DESIGNER') || c.includes('GERENTE DE MARKETING') || c.includes('GERENTE DE CONTEÚDO')) {
...
export function CentralDeEmpregosTab({ gameState, onUpdateGameState, triggerNotification, theme }: CentralDeEmpregosTabProps) {
  ...
  useEffect(() => {
    let initialEmployees = gameState.corporationStaffEmployees;
    if (!initialEmployees || initialEmployees.length === 0) {
      const saved = localStorage.getItem('legendshub_corporation_staff');
      if (saved) {
        try {
          initialEmployees = JSON.parse(saved);
        } catch (e) {
          initialEmployees = [];
        }
      }
    ...
    if (!gameState.corporationStaffJobPool || gameState.corporationStaffJobPool.length === 0) {
      onUpdateGameState({
        ...
        corporationStaffEmployees: initialEmployees
      });
    } else {
  ...
  const applyForJob = (targetTeam: Team) => {
    ...
    const updatedManagers = (gameState.managers || []).map(m => {
      if (m.teamId === targetTeam.id && m.id !== 'player-manager') {
        return { ...m, teamId: null, status: 'Free Agent' };
      }
      // Set player manager to the target team
      if (m.id === 'player-manager') {
        return { ...m, teamId: targetTeam.id };
      }
      return m;
    });
  ...
  return (
    <div className={`space-y-6 select-none font-sans text-xs p-6 rounded-2xl border ${isDark ? 'bg-[#0a1424] text-slate-100 border-[#1e2d44]' : 'bg-white text-slate-800 border-slate-200'}`}>
      {/* Visual Welcome Banner */}
      <div className={`p-5 rounded-2xl flex items-center justify-between border shadow-sm ${isDark ? 'bg-[#070d19]/60 border-[#1e2d44]/60' : 'bg-slate-50 border-slate-200'}`}>
        <div>
          <h3 className={`font-display text-sm font-black uppercase tracking-wider flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <Briefcase className="w-5 h-5 text-cyan-600 shrink-0" />
            Central Profissional de Oportunidades & Empregos
          </h3>
          <p className={`text-[10.5px] mt-1.5 max-w-xl leading-relaxed ${isDark ? 'text-slate-450' : 'text-slate-650'}`}>
            Monitore o mercado corporativo da Superliga e staff de especialistas. Transite livremente entre clubes sob status de <strong className={`font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>Free Agent (Desempregado)</strong> ou comande o RH de sua organização!
          </p>
        </div>
      ...
      {playerTeam ? (
        <div className={`p-4 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border ${isDark ? 'bg-rose-950/20 border-rose-900/30' : 'bg-rose-50 border-rose-100'}`}>
          <div>
            <h4 className={`font-extrabold uppercase tracking-wide text-xs flex items-center gap-1.5 ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>
            ...
            <p className={`text-[10px] mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Se você desejar abandonar suas obrigações táticas e finanças atuais para voltar ao mercado como profissional livre (Free Agent), solicite a rescisão contratual imediata.</p>
          </div>
          <button 
            onClick={() => {
        ...
        <div className={`p-4 rounded-xl flex items-center gap-3 border ${isDark ? 'bg-[#1a140f] border-amber-900/40' : 'bg-amber-50 border-amber-200'}`}>
          <AlertCircle className="w-6 h-6 text-amber-600 shrink-0" />
          <div>
            <h4 className={`font-extrabold uppercase tracking-wide text-xs ${isDark ? 'text-amber-400' : 'text-amber-800'}`}>Você está Atualmente Operando como Free Agent (Desempregado)</h4>
            <p className={`text-[10.5px] mt-0.5 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>As rodadas da Superliga CBLOL continuam simuladas dinamicamente a cada clique de avançar semana. Escolha uma das vagas disponíveis abaixo na Central de Times para assinar contrato e iniciar seu novo legado!</p>
          </div>
        </div>
      )}

      {/* Departments Horizontal Custom Tabs bar */}
      <div className={`border-b flex items-center gap-1 overflow-x-auto pb-1 scrolled-mini ${isDark ? 'border-[#1e2d44]' : 'border-slate-200'}`}>
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg flex items-center gap-2 transition whitespace-nowrap border cursor-pointer shrink-0 ${
                isActive
                  ? (isDark ? 'bg-[#00E5FF]/10 border-[#00E5FF]/30 text-[#00E5FF] font-black shadow-sm' : 'bg-cyan-50 border-cyan-200 text-cyan-600 font-extrabold shadow-sm')
                  : (isDark ? 'bg-transparent border-transparent text-slate-400 hover:text-slate-200 hover:bg-[#1a2d44]/20' : 'bg-transparent border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-100')
              }`}
            >
              <CatIcon className={`w-3.5 h-3.5 ${isActive ? (isDark ? 'text-[#00E5FF]' : 'text-cyan-600') : 'text-slate-500'}`} />
              {cat.label}
              <span className={`px-1.5 py-0.5 rounded-full text-[8px] font-mono ${
                isActive 
                  ? (isDark ? 'bg-[#00E5FF]/25 text-[#00E5FF]' : 'bg-cyan-100 text-cyan-700') 
        ...
        {activeTab === 'VAGAS' ? (
          <div>
            <div className="flex justify-between items-center mb-3">
              <h4 className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-700'}`}>Assinatura de Times Disponíveis na Liga (Zonas Críticas)</h4>
            </div>
              ...
              {availableBids.map((t) => {
                ...
                return (
                  <div key={t.id} className={`p-4 rounded-xl border flex flex-col justify-between space-y-4 shadow-sm transition ${
                    isCurrent 
                      ? (isDark ? 'bg-[#00E5FF]/5 border-[#00E5FF]/30 text-white' : 'bg-cyan-50/50 border-cyan-200 text-slate-800') 
                      : (isDark ? 'bg-[#0f172a] border-[#1e2d44] hover:border-slate-700 hover:shadow-md text-slate-250' : 'bg-white border-slate-200 hover:border-slate-300 hover:shadow-md text-slate-800')
                  }`}>
                    <div className="flex justify-between items-start">
                      <div className="flex items-start gap-3">
                        <div 
                        ...
                        <div>
                          <h4 className={`font-extrabold text-[11.5px] uppercase tracking-wide flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            {t.name}
                            <span className={`text-[8px] px-1.5 py-0.5 rounded font-mono border ${isDark ? 'bg-slate-900 border-slate-700 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-650'}`}>
                              {t.region || 'CBLOL'}
                            </span>
                          </h4>
                          <div className="text-[9px] text-slate-500 mt-1 font-mono flex flex-wrap gap-2.5">
                            <span>Caixa: <strong className="text-emerald-500 font-bold">$ {t.budget.toLocaleString()}</strong></span>
                            <span>•</span>
                            <span>Líder Popularidade: <strong className="text-cyan-500 font-bold">{t.popularity}%</strong></span>
                          </div>
                        </div>
                      </div>
                      ...
                      <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full font-mono shrink-0 border ${
                        rank <= 4 
                          ? (isDark ? 'bg-blue-950/25 border-blue-900/30 text-blue-400' : 'bg-blue-50 border-blue-105 text-blue-700') 
                          : (isDark ? 'bg-amber-955/25 border-amber-900/30 text-amber-400' : 'bg-amber-50 border-amber-105 text-amber-700')
                      }`}>
                        {rank}º Colocado ({t.wins}V - {t.losses}D)
                      </span>
                    </div>

                    <p className={`text-[10px] leading-normal border-t pt-3 ${isDark ? 'text-slate-400 border-[#1e2d44]' : 'text-slate-650 border-slate-100'}`}>
                    ...
                    <div className={`flex items-center justify-between border-t pt-3.5 ${isDark ? 'border-[#1e2d44]' : 'border-slate-150'}`}>
                      <div className="space-y-0.5">
                        <span className="text-[7.5px] text-slate-450 uppercase font-bold block leading-none">Vínculo Inicial</span>
                        <span className={`text-[10px] font-extrabold uppercase font-mono ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>Garantido: 1 Split (32 sem)</span>
                      </div>
                      
                      {isCurrent ? (
                        <span className={`px-3.5 py-1.5 border text-[9px] font-black uppercase tracking-widest rounded-lg ${isDark ? 'bg-[#00E5FF]/10 border-[#00E5FF]/25 text-[#00E5FF]' : 'bg-cyan-50 border-cyan-200 text-cyan-600'}`}>
                          CLUBE ATUAL
                        </span>
                      ) : (
                        <button 
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <h4 className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-700'}`}>Alinhamento de Candidatos para o Setor do Gaming Office</h4>
            </div>
            ...
            {candidatesFiltered.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {candidatesFiltered.map((candidate) => {
                  ...
                  return (
                    <div key={candidate.id} className={`p-4 rounded-xl flex flex-col justify-between gap-4 border shadow-sm hover:shadow transition ${
                      isDark 
                        ? 'bg-[#0f172a] border-[#1e2d44] hover:border-slate-700 text-slate-200' 
                      ...
                      <div className="flex items-start gap-3.5">
                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center border shrink-0 ${
                          ...
                          {getStaffIcon(candidate.cargo, candidate.departamento)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className={`font-extrabold text-[11.5px] uppercase truncate tracking-wide ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            {candidate.nome}
                            <span className={`text-[9px] font-normal ml-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                              ({candidate.idade || 26} anos • {candidate.bandeira || '🇧🇷'} {candidate.nacionalidade || 'BR'})
                            </span>
                          </h4>
                          <span className={`text-[8.5px] font-mono tracking-widest px-1.5 py-0.5 rounded inline-block mt-0.5 font-bold uppercase ${
                            isDark ? 'bg-[#00E5FF]/10 text-[#00E5FF] border border-[#00E5FF]/20' : 'bg-cyan-50 text-cyan-700'
                          }`}>
                            {candidate.cargo}
                          </span>

                           <p className={`text-[10px] italic mt-1.5 leading-tight ${isDark ? 'text-slate-400' : 'text-slate-650'}`}>
                             Especialidade: <strong className={isDark ? 'text-slate-200' : ''}>{candidate.especialidade || 'Trabalho de foco tático e prevenção'}</strong>
                           </p>

                           <div className="flex items-center gap-1.5 mt-2">
                             <span className={`text-[9.5px] font-bold ${isDark ? 'text-slate-400' : 'text-slate-550'}`}>Classificação:</span>
                             <div className="flex items-center text-amber-500 shrink-0">
                               {Array.from({ length: 5 }).map((_, idx) => {
                                 if (idx < fullStars) {
                                   ...
                                   return <Star key={idx} className={`w-3 h-3 ${isDark ? 'text-slate-700' : 'text-slate-200'}`} />;
                                 }
                               })}
                             </div>
                           </div>
                      ...
                      <div className={`border-t pt-3 flex items-center justify-between ${isDark ? 'border-[#1e2d44]' : 'border-slate-100'}`}>
                        <div className="space-y-0.5">
                          <span className={`text-[7.5px] uppercase block font-bold leading-none ${isDark ? 'text-slate-450' : 'text-slate-400'}`}>Salário Exigido</span>
                          <span className={`text-xs font-mono font-black block leading-none ${isDark ? 'text-rose-400' : 'text-rose-600'}`}>$ {candidate.salario_semanal.toLocaleString('pt-BR')}/sem</span>
                        </div>

                        {playerTeamId === '' ? (
                          <div className={`text-[8px] font-bold px-2 py-1 rounded flex items-center gap-1.5 cursor-not-allowed border ${
                            isDark ? 'bg-slate-900 border-slate-800 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-550'
                          }`}>
                            <Lock className="w-3 h-3 text-slate-400 shrink-0" />
                            Apenas Managers ativos podem recrutar
                          </div>
                        ) : playerTeam && playerTeam.budget < candidate.salario_semanal * 2 ? (
                          <div className={`text-[8px] font-bold px-2 py-1 rounded cursor-not-allowed border ${
                            isDark ? 'bg-rose-950/20 border-rose-900/30 text-rose-450' : 'bg-rose-50 border-rose-100 text-rose-600'
                          }`}>
              ...
              <div className={`text-center py-10 border border-dashed rounded-2xl flex flex-col items-center justify-center gap-2 ${
                isDark ? 'border-[#1e2d44] bg-[#070d19]/40' : 'border-slate-200 bg-slate-50/50'
              }`}>
                <Users className="w-8 h-8 text-slate-400 shrink-0 animate-pulse" />
                <h5 className={`text-[10px] uppercase font-black tracking-widest ${isDark ? 'text-slate-350' : 'text-slate-450'}`}>Nenhum Candidato Qualificado Disponível</h5>
                <p className={`text-[9px] max-w-sm mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Nenhum especialista está solicitando entrevistas para este setor na semana atual da Superliga. Aguarde a rodagem das rodadas.</p>
              </div>
            )}
          </div>
        )}
