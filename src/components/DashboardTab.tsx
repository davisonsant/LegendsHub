...
const TRANSLATIONS = {
  ...
  es: {
    ...
    teamMatches: "Partidos del Club",
    mainTeam: "EQUIPO PRINCIPAL",
  }
};
...
export default function DashboardTab({
  ...
  const [showManagerGuide, setShowManagerGuide] = useState(() => {
    const hidden = localStorage.getItem('legendshub_hide_guide') === 'true';
    if (hidden) return false;
    return localStorage.getItem('legendshub_guided_seen') !== 'true';
  });
  
  const getChampAvatarUrl = (champ: any) => {
    const customUrl = getGameAssetUrl('champions', champ.id);
    ...
    const seed = typeof champ.imageSeed === 'number' ? champ.imageSeed : 0;
  ...
  const { week, stage, season, teams, playerTeamId, currentPatch, roundsPlayedThisWeek, champions } = gameState;
  const playerTeam = teams.find(t => t.id === playerTeamId) || teams[0];
  const activeRegion = playerTeam.region || 'CBLOL';

  // Obter um rival de alta popularidade na mesma região, excluindo o próprio time do jogador
  const sameRegionTeams = teams.filter(t => t.region === playerTeam.region && t.id !== playerTeam.id);
  const rivalTeam = sameRegionTeams.sort((a, b) => (b.popularity || 0) - (a.popularity || 0))[0] || { id: 'rival', name: 'paiN Gaming', acronym: 'PNG', logoUrl: '' };
  ...
  // Sincronização offline e blobs com default.db
  if (playerTeam) {
    playerTeam.logoUrl = getGameAssetUrl('teams', playerTeam.id, playerTeam.logoUrl);
  }
  if (opponentTeamObj) {
    opponentTeamObj.logoUrl = getGameAssetUrl('teams', opponentTeamObj.id, opponentTeamObj.logoUrl);
  }

  const currentOpponent = opponentTeamObj || rivalTeam;
  const opponentName = currentOpponent.name;
  const opponentAcronym = currentOpponent.acronym;
  const isRival = opponentTeamObj ? opponentTeamObj.id === rivalTeam.id : true;
  const rivalName = opponentName;
  const rivalAcronym = opponentAcronym;
  ...
  const athleteCosts = [...(playerTeam.roster || []), ...(playerTeam.substitutes || [])].reduce((acc, p) => acc + p.salary / 4, 0);
  ...
  const proceduralTweets = [
    {
      ...
      content: {
        pt: `O clássico ${playerTeam.name} contra ${rivalAcronym} nessa rodada nesta semana vai parar o cenário competitivo de eSports! Disputa acirradíssima pelo topo da liga. Quem perder vai aguentar sarro da torcida rival. Qual seu palpite? 🔥`,
        en: `The classic match of ${playerTeam.name} vs ${rivalAcronym} this week will stop the competitive eSports scene! Extremely fierce battle for the top of the league. Whoever loses will be mocked by the rival fans. What is your guess? 🔥`,
        es: `¡El partido clásico de ${playerTeam.name} contra ${rivalAcronym} esta semana detendrá la escena competitiva de los eSports! Batalla extremadamente feroz por la cima de la liga. El que pierda será objeto de burlas por parte de la afición rival. ¿Cuál es tu pronóstico? 🔥`
      }[lang],
      likes: 8520,
      retweets: 3200,
    ...
    },
    {
      ...
      content: {
        pt: `${playerTeam.name} contra ${rivalAcronym} nos palcos... se vacilarem na fase de rotas ou errarem o pick de escala do patch, já era. Esse duelo de liderança vai ser decidido nos detalhes técnicos! 👊`,
        en: `${playerTeam.name} vs ${rivalAcronym} on stage... if they slip up in laning phase or pick the wrong scaling champ for the patch, it's over. This leadership duel will be decided on technical details! 👊`,
        es: `${playerTeam.name} contra ${rivalAcronym} en el escenario... si flaquean en la fase de líneas o eligen al campeón equivocado para el parche, se acabó. ¡Este duelo por el liderato se decidirá en los detalles técnicos! 👊`
      }[lang],
      likes: 9340,
      retweets: 4100,
    ...
    },
    {
      ...
      content: {
        pt: `Análise fria: ${playerTeam.name} (${playerTeam.wins}-${playerTeam.losses}) está demonstrando ótimas transições de macro, mas a ${rivalAcronym} chega embalada. Jogo chave pra ver quem é o verdadeiro líder de ritmo.`,
        en: `Cold review: ${playerTeam.name} (${playerTeam.wins}-${playerTeam.losses}) is showing great macro transitions, but ${rivalAcronym} arrives on fire. Key game to see who is the true rhythm leader.`,
        es: `Análisis frío: ${playerTeam.name} (${playerTeam.wins}-${playerTeam.losses}) está demostrando excelentes transiciones macro, pero ${rivalAcronym} llega encendida. Partido clave para ver quién es el verdadero líder de ritmo.`
      }[lang],
      likes: 5690,
      retweets: 1250,
    ...
    <div className={s.bgPage}>
        ...
        <div className={`p-3.5 rounded-xl border flex items-center gap-3 overflow-hidden ${
          ...
          <div className="text-xs font-bold truncate leading-none">
            ...
            <span className={s.textWhiteOrSlate}>
              {lang === 'pt' && `${playerTeam.name} (${playerTeam.wins}V-${playerTeam.losses}D) e ${rivalName} duelam no El Clásico valendo a liderança isolada. Bull tem 2 semanas de contrato restante, nova proposta Drakos disponível!`}
              {lang === 'en' && `${playerTeam.name} (${playerTeam.wins}W-${playerTeam.losses}L) and ${rivalName} duel for the crown at El Clásico. Bull has only 2 weeks left on contract! Drakos offer ready.`}
              {lang === 'es' && `${playerTeam.name} (${playerTeam.wins}V-${playerTeam.losses}D) y ${rivalName} se enfrentan en El Clásico por el liderato. ¡A Bull le quedan 2 semanas de contrato! Drakos disponible.`}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
              ...
              <div className="space-y-2.5 text-xs font-medium">
                ...
                <div className="flex justify-between items-center bg-blue-600/10 p-2 rounded border border-blue-500/20">
                  ...
                  <span className="font-bold text-rose-500">
                    {lang === 'pt' 
                      ? (isRival ? `El Clásico contra ${opponentName}` : `Partida Oficial contra ${opponentName}`)
                      : lang === 'es'
                        ? (isRival ? `El Clásico contra ${opponentName}` : `Partido Oficial contra ${opponentName}`)
                        : (isRival ? `El Clásico against ${opponentName}` : `Official Match against ${opponentName}`)
                    }
                  </span>
                </div>
              </div>
            ...
            >
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100 dark:border-slate-800/80">
                <div className="flex items-center gap-2 text-amber-500">
                  ...
                  <span className="font-display text-xs font-black uppercase tracking-wider text-slate-400">PATCH META 15.1</span>
                </div>
                <span className="text-[9px] px-1.5 py-0.5 bg-amber-500/10 text-amber-600 font-mono font-bold rounded">META S+</span>
              </div>
              
              <div className="space-y-3 flex-1 flex flex-col justify-between">
                <div>
                  <p className="text-[10px] font-mono font-black text-amber-500 uppercase tracking-widest mb-2.5">
                    CAMPEÕES TIER S+ NO PATCH
                  </p>
                  
                  {(() => {
                    const sPlusChamps = (champions || []).filter((c: any) => c.tier === 'S+');
                    return sPlusChamps.length > 0 ? (
                      <div className="grid grid-cols-6 gap-2.5">
                        {sPlusChamps.slice(0, 12).map((champ: any) => {
                          ...
                          return (
                            ...
                            >
                              <div className="w-8 h-8 rounded-full overflow-hidden border border-amber-500/35 group-hover/item:border-amber-500 transition-colors shadow-sm shadow-amber-500/5 relative shrink-0 bg-slate-900 flex items-center justify-center">
                                <img 
                                  src={champion?.imageUrl || (champion?.image_blob ? getImageUrl(champion.image_blob) : getChampAvatarUrl(champion))} 
                      ...
                      <p className={`${s.textMuted} text-[10px] font-sans italic`}>
                        Nenhum campeão editado como S+ no momento.
                      </p>
                    );
                  })()}
                </div>

                <p className="text-[10px] text-right text-slate-400 font-bold uppercase tracking-wider group-hover:text-amber-500 transition-colors pt-2.5">
                  {t.analyzeMeta}
                </p>
              </div>
            </div>
          ...
          {/* NOVO CARD DE JOGOS DA RODADA (Time Principal & Academy) */}
          <div className={`${s.bgCardNoFlex} animate-fade-in`}>
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100 dark:border-slate-800/80">
              <div className="flex items-center gap-2 text-cyan-500">
                <Calendar className="w-4 h-4" />
                <span className="font-display text-xs font-black uppercase tracking-wider text-slate-400">{t.teamMatches} ({lang === 'en' ? 'Week' : lang === 'es' ? 'Semana' : 'Semana'} {week})</span>
              </div>
              <span className="text-[9px] px-1.5 py-0.5 bg-cyan-500/10 text-cyan-600 font-mono font-bold rounded">LIVE CONFRONTOS</span>
            </div>

            {!playerNextOpponentMatch ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <span className="text-3xl mb-2">⭐</span>
                <p className="text-xs font-black uppercase text-slate-400 dark:text-slate-505 tracking-wider">
                  Nenhum confronto oficial nesta semana
                </p>
                <p className="text-[10px] text-slate-500 dark:text-slate-500 font-semibold mt-1">
                  Aproveite este período de offseason para planejar estratégias e treinar seu elenco.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Confronto Principal */}
                <div className={`flex flex-col items-center justify-center p-3 rounded-xl border shadow-inner ${
                  isDark ? 'bg-slate-900/40 border-slate-800' : 'bg-slate-100/50 border-slate-200/40'
                }`}>
                  <span className="text-[9px] font-mono font-black text-cyan-500 uppercase tracking-widest mb-2.5">{t.mainTeam}</span>
                  <div className="flex items-center gap-4">
                    {/* Time do Jogador */}
                    {playerTeam?.logoUrl ? (
                      <div className={`w-10 h-10 p-1 flex items-center justify-center rounded-full border shadow-sm ${
                      ...
                      <div className="w-10 h-10 rounded-full bg-emerald-500/15 text-emerald-500 font-mono font-black text-xs flex items-center justify-center shadow-inner border border-emerald-500/20">
                        {playerTeam.acronym}
                      </div>
                    )}

                    <span className={`text-xs font-mono font-black ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>X</span>
                    ...
                    {opponentTeamObj ? (
                      opponentTeamObj.logoUrl ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-slate-500/15 text-slate-400 font-mono font-black text-xs flex items-center justify-center shadow-inner border border-slate-800/50">
                          {opponentTeamObj.acronym}
                        </div>
                      )
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-slate-500/15 text-slate-400 font-mono font-black text-xs flex items-center justify-center shadow-inner border border-slate-800/50">
                  ...
                  <span className={`text-[8.5px] font-mono mt-2.5 px-1.5 py-0.5 rounded leading-none ${
                    isDark ? 'text-slate-400 bg-slate-900/50' : 'text-slate-400 bg-slate-200/50'
                  }`}>
                    DOMINGO • 13h00
                  </span>
                </div>
                ...
                <div className={`flex flex-col items-center justify-center p-3 rounded-xl border shadow-inner ${
                  ...
                  <span className="text-[9px] font-mono font-black text-purple-500 uppercase tracking-widest mb-2.5">ACADEMY</span>
                  <div className="flex items-center gap-4">
                    {/* Time do Jogador Base / Academy */}
                    {playerTeam?.logoUrl ? (
                      <div className={`w-10 h-10 p-1 flex items-center justify-center rounded-full border shadow-sm relative ${
                        isDark ? 'bg-slate-950 border-purple-800/60' : 'bg-white border-purple-500/30'
                      }`}>
                        <img 
                          src={playerTeam.logoUrl} 
                          alt={playerTeam.name} 
                          ...
                          onError={(e) => { e.currentTarget.src = 'assets/ui/fallback-logo.png'; }}
                        />
                        <span className="absolute -bottom-0.5 -right-0.5 bg-purple-600 text-white font-mono text-[6px] font-black w-3.5 h-3.5 rounded-full flex items-center justify-center border border-slate-950">A</span>
                      </div>
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-purple-500/15 text-purple-400 font-mono font-black text-xs flex items-center justify-center shadow-inner border border-purple-500/20">
                        {playerTeam?.acronym}A
                      </div>
                    )}

                    <span className={`text-xs font-mono font-black ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>X</span>

                    {/* Oponente Base / Academy */}
                    {opponentTeamObj ? (
                      opponentTeamObj.logoUrl ? (
                        <div className={`w-10 h-10 p-1 flex items-center justify-center rounded-full border shadow-sm relative ${
                          isDark ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'
      ...
      {showManagerGuide && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in text-left">
          <div className="w-full max-w-2xl border bg-[#0a1424] border-[#1e2d44] rounded-2xl p-7 shadow-2xl relative overflow-hidden">
            {/* Absolute accent glow */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl pointer-events-none" />

            <div className="flex items-center gap-2.5 mb-2 border-b border-white/5 pb-4">
              <div className="p-2 bg-blue-600/15 text-cyan-400 rounded-xl">
            ...
            <div className="space-y-4 max-h-[360px] overflow-y-auto pr-1 scrollbar-thin my-5 leading-relaxed text-xs">
              <div className="p-3.5 rounded-xl bg-slate-900/60 border border-[#1e2d44]/55 flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-100">Dashboard / Central</h4>
            ...
            <div className="pt-5 border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <label className="flex items-center gap-2.5 text-[11px] font-semibold text-slate-300 cursor-pointer hover:text-white select-none">
                <input 
                  ...
                  className="w-4 h-4 rounded border-[#1e2d44] bg-[#0c1424] text-cyan-500 focus:ring-0 focus:ring-offset-0 cursor-pointer"
                />
                <span>{lang === 'es' ? 'No volver a mostrar esta guía' : lang === 'en' ? "Don't show this guide again" : 'Não exibir este guia novamente'}</span>
              </label>

              <button
                onClick={() => {
                  if (dontShowAgain) {
                    localStorage.setItem('legendshub_hide_guide', 'true');
                  }
                  setShowManagerGuide(false);
                  localStorage.setItem('legendshub_guided_seen', 'true');
                }}
                className="bg-blue-600 hover:bg-blue-700 text-white font-mono font-black text-[10.5px] uppercase tracking-widest py-3 px-6 rounded-lg cursor-pointer transition-all shadow-md shadow-blue-500/15"
              >
                {lang === 'es' ? '¡Entendido, Vamos al Jogo!' : lang === 'en' ? "Understood, Let's Play!" : 'Entendido, Vamos ao Jogo!'}
              </button>
            </div>
          </div>
        </div>
